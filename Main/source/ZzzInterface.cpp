...
void RenderNotices()
{
	...
	for (int i = 0; i < MAX_NOTICE; i++)
	{
		...
		{
			g_pRenderText->RenderText(setPosCenterX(0), (GetWindowsY - 180) + i * 13, n->Text, 0, 0, RT3_WRITE_CENTER);
		}
		else
		{
			g_pRenderText->RenderText(setPosCenterX(0), (GetWindowsY - 180) + i * 13, n->Text, 0, 0, RT3_WRITE_CENTER);
...
void RenderBoolean(int x, int y, CHAT* c)
{
	...
	if (c->Owner->GuildMarkIndex >= 0 && c->Owner->Object.Type == MODEL_PLAYER)
	{
		float posX = (float)x - 21.0f;

		CreateGuildMark(c->Owner->GuildMarkIndex);

		::RenderBitmap(BITMAP_GUILD, posX, (float)y, 16.0f, 16.0f, 0.0f, 0.0f, 1.0f, 1.0f, 1, 1, 0);
	}
...
bool CheckDoubleControlForAutoPVP()
{
	static int iLastControlTime = 0;
	static int iControlCount = 0;
	const int DOUBLE_DELAY = 300;

	int currentTime = GetTickCount();

	if (SEASON3B::IsPress(VK_CONTROL))
	{
		if (currentTime - iLastControlTime <= DOUBLE_DELAY)
		{
			iControlCount++;
			if (iControlCount >= 2)
			{
				g_bAutoPVP = !g_bAutoPVP;

				g_pChatListBox->AddText("",
					g_bAutoPVP ? "System [2X Ctrl] AutoPVP [ON]" : "System [2X Ctrl] AutoPVP [OFF]",
...
bool CheckAttack_Fenrir(CHARACTER* c)
{
	...
	else if (c->Object.Kind == KIND_PLAYER)
	{
		...
		else if (c->PK >= PVP_MURDERER2 || ((HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128 || g_bAutoPVP) && c != Hero))
		{
			return true;
		}
...
int getTargetCharacterKey(CHARACTER* c, int selected)
{
	...
	else if (sc->PK >= PVP_MURDERER2 || ((HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128 || g_bAutoPVP) && sc != Hero))
	{
		return sc->Key;
	}
...
#else
					//SendPosition( positionX, positionY );	//	Aqui desativa o pux�o da skill
#endif
			}
		}
	}
...
void CheckChatText(char* Text)
{
	...
	else if (FindText(Text, GlobalText[2800]))
	{
...
void MoveHero()
{
	...
	int Angle;
	bool bLookAtMouse = (!gCamera.m_CameraOnOff); bool NoAutoAttacking = false;
				...
				if (Success && c->Object.Live)
				{
					...
					if (!c->Movement || (abs((c->PositionX) - xPos) < 2 && abs((c->PositionY) - yPos) < 2))
					{
							...
							PathFinding2((c->PositionX), (c->PositionY), TargetX, TargetY, &c->Path))
						{
							...
							{
								// [2026-08-30] Visual S6: pino 3D do destino ao andar (MoveTargetPosEffect).
...
void RenderOutSides()
{
	if (gMapManager.WorldActive == WD_8TARKAN)
	{
		...
		//RenderBitmapUV(BITMAP_CHROME + 2, 0.f, 0.f, 640.f + GWidescreen.DisplayWinCDepthBox, 640.f, WindX, 0.f, 0.3f, 0.3f);
		float WindX2 = (float)((int)WorldTime % 100000) * 0.001f;
		//RenderBitmapUV(BITMAP_CHROME + 3, 0.f, 0.f, 640.f + GWidescreen.DisplayWinCDepthBox, 640.f, WindX2, 0.f, 3.f, 2.f);
	}
