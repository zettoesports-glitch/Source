...
bool SEASON3B::CNewUISystem::Create()
{
	...
	if (false == m_pNewChatLogWindow->Create(m_pNewUIMng, 0, GetWindowsY - 98.f, 6))
		return false;

	m_pNewOptionWindow = new CNewUIOptionWindow;
	if (m_pNewOptionWindow->Create(m_pNewUIMng, (int)163 + GetCenterX(640), 70) == false)
	{
		return false;
	}
...
bool SEASON3B::CNewUISystem::LoadMainSceneInterface()
{
	...
	if (false == m_pNewChatInputBox->Create(m_pNewUIMng, m_pNewChatLogWindow, 0, GetWindowsY - 98.f))
	{
		return false;
	}
	SetFocus(g_hWnd);
	...
	if (m_pNewWindowMenu->Create(m_pNewUIMng, (GetWindowsX / 2) + 208, GetWindowsY - 175) == false)
	{
		return false;
	}
	...
	if (m_pNewSiegeWarfare->Create(m_pNewUIMng, GetWindowsX - 154, GetWindowsY - 246) == false)
		return false;
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if (m_pNewUIMng)
	{
		if (dwKey == SEASON3B::INTERFACE_FRIEND)
		{
			...
			m_pNewFriendWindow->OpenMainWnd(GetWindowsX - 250, setPosDown(432 - 173));
		}
		...
		else if (dwKey == SEASON3B::INTERFACE_RANKING_TOP)
		{
