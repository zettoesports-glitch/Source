...
bool SEASON3B::CNewUIButton::Render( bool RendOption )
{
	if( m_ButtonInfo.size() != 0 )
	{
		if (m_CurImgIndex == 43592)
		{
			if (m_CurImgState == 0)
			{
				RenderImage(m_CurImgIndex, m_Pos.x, m_Pos.y, 19, 19, 0, 19.f);
			}
			else
			{
				RenderImage(m_CurImgIndex, m_Pos.x, m_Pos.y, 19, 19, 0, 38.f);
			}
		}
		else
		{
	...
	if( m_TooltipText.size() != 0 )
	{
		if( CheckMouseIn(m_Pos.x, m_Pos.y, m_Size.x, m_Size.y) )
		{
			...
			RenderText( m_TooltipText.c_str(), x, y, Fontsize.cx+15, 0, m_hToolTipFont, m_TooltipTextColor, RGBA(0, 0, 0, 180), RT3_SORT_CENTER );
		}
	}
