...
void CLoginWin::Create()
{
    ...
    if (!m_bAfterLogout)
    {
        GetPrivateProfileString("AutoLogin", "User", "", Acc, 32, ".\\Data\\Custom\\config.ini");
        GetPrivateProfileString("AutoLogin", "Password", "", Pass, 32, ".\\Data\\Custom\\config.ini");
    }
    else
    {
        Acc[0] = '\0';
...
void CLoginWin::Show(bool bShow)
{
    ...
    if (bShow && m_bAfterLogout)
    {
        m_pIDInputBox->SetText("");
        m_pPassInputBox->SetText("");
        m_bAfterLogout = false;
    }
}
...
void CLoginWin::UpdateWhileActive(double dDeltaTick)
{
    ...
    if (this->FirstLoad == 1)
    {
        ...
        int AutoLoginnn = GetPrivateProfileInt("AutoLogin", "AutoLoginEnable", 0, ".\\Data\\Custom\\config.ini");

        if (LogIn == 0 && unicode::_strlen(szID) > 0 && unicode::_strlen(szPass) > 0 && !m_bAfterLogout && AutoLoginnn == 1)
        {
...
void CLoginWin::RenderControls()
{
    if (this->FirstLoad == 1)
    {
        ...
        m_pIDInputBox->GetText(szID, MAX_ID_SIZE + 1);
        m_pPassInputBox->GetText(szPass, MAX_PASSWORD_SIZE + 1);
        int AutoLoginnn = GetPrivateProfileInt("AutoLogin", "AutoLoginnn", 0, ".\\Data\\Custom\\config.ini");

        if (unicode::_strlen(szID) > 0 && unicode::_strlen(szPass) > 0 && !m_bAfterLogout && AutoLoginnn == 1)
        {
            return;
        }
        if (unicode::_strlen(szID) > 0)
...
void CLoginWin::RequestLogin()
{
    ...
    {
        WritePrivateProfileString("AutoLogin", "User", szID, ".\\Data\\Custom\\config.ini");
        WritePrivateProfileString("AutoLogin", "Password", szPass, ".\\Data\\Custom\\config.ini");
...
{
    m_bAfterLogout = true;
    WritePrivateProfileString("AutoLogin", "User", "", ".\\Data\\Custom\\config.ini");
    WritePrivateProfileString("AutoLogin", "Password", "", ".\\Data\\Custom\\config.ini");
    m_pIDInputBox->SetText("");
    m_pPassInputBox->SetText("");
