	...
	if (c->Object.Type == MODEL_PLAYER)
	{
		if ((c->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1) && !c->SafeZone)
		{
			if (c->MiniatureMode)
			{
				...
				{
					// Sem Wing ? animação normal (permite skills de espada)
...
#endif
					{
						if (c->Weapon[0].Type != -1 || c->Weapon[1].Type != -1)
						{
							// Usa animação com arma para permitir skills como Sword4
			...
			{
				// === FENRIR NORMAL (sem miniatura) - Código original ===
#ifdef PBG_ADD_NEWCHAR_MONK_ANI
				if (GetBaseClass(c->Class) == CLASS_RAGEFIGHTER)
				{
					if (c->Weapon[0].Type != -1 && c->Weapon[1].Type != -1)
		...
		else if (c->SafeZone && c->Weapon[1].Type == MODEL_HELPER + 5 && !gMapManager.InChaosCastle())
		{
...
//				    SetAction(&c->Object,PLAYER_FLY_RIDE_WEAPON);
	...
	else if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD && (c->Helper.Type == MODEL_HELPER + 4 || gHelperSystem.CheckHelperType(c->Helper.Type, 16) == 1) && !c->SafeZone)
	{
...
bool AttackStage(CHARACTER* c, OBJECT* o)
{
	...
	{
		if (g_pOption->m_DisableSkill == 1)
			g_iLimitAttackTime = 30;
		else
			g_iLimitAttackTime = 1;
	}
	else
	{
		g_iLimitAttackTime = 1;
	...
	case AT_SKILL_REDUCEDEFENSE:
		if (o->Type == MODEL_PLAYER)
		{
			...
			if (o->AnimationFrame >= 3.f && rand_fps_check(1))
			{
				...
				{
					vec3_t Offset1 = { o->Position[0] + 10.0f, o->Position[1], o->Position[2] };
					vec3_t Offset2 = { o->Position[0] - 10.0f, o->Position[1], o->Position[2] };
	...
	case AT_SKILL_ICE_BLADE:
		if (o->Type == MODEL_PLAYER && o->CurrentAction == PLAYER_ATTACK_TWO_HAND_SWORD_TWO)
		{
			...
			{
				Angle[2] -= 40.f;
				CreateEffect(MODEL_MAGIC2, o->Position, Angle, o->Light, 5, o);
				Angle[2] += 20.f;
				CreateEffect(MODEL_MAGIC2, o->Position, Angle, o->Light, 5, o);
				Angle[2] += 20.f;
				CreateEffect(MODEL_MAGIC2, o->Position, Angle, o->Light, 5, o);
...
void DeadCharacter(CHARACTER* c, OBJECT* o, BMD* b) // Th?i gian ch?t
{
	if (c->Dead <= 0) return;
	...
	// Gi?m th?i gian ch?t t? 1.0 xu?ng 0.3 giây
	float RotTime = 0.7f;
	if (c->Rot >= RotTime)
	{
		if (o->Type != MODEL_MONSTER01 + 116)
		{
			// Gi?m Alpha nhanh hon d? bi?n m?t s?m
			o->Alpha = 1.f - (c->Rot - RotTime) * 3;
		}

		if (o->Alpha >= 0.1f)
		{
			// Ð?y quái xu?ng nhanh hon
			o->Position[2] -= 1.0f;
		}
		else if (c != Hero)
		{
...
void MoveCharacter(CHARACTER* c, OBJECT* o)
{
	...
	if (c->AttackTime >= g_iLimitAttackTime)
	{
		...
		case AT_SKILL_VITALITY:
		{
			...
			for (int i = 0; i < iCount; ++i)
			{
				...
				if (g_pOption->m_NSkillCollor)
				{
					...
					CreateJoint(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 23, o, 1.9f, 0, 0);
					if ((i % 20) == 0)
					{
						CreateEffect(BITMAP_MAGIC + 1, o->Position, Angle, o->Light, 6, o);
					}
				}
				else
				{
		...
		case AT_SKILL_LIGHTNING_SHOCK:
		{
			...
			CreateEffectFpsChecked(MODEL_LIGHTNING_SHOCK, o->Position, o->Angle, vLight, 0, o);
...
void MoveCharacterPosition(CHARACTER* c)
{
	...
	{
		o->Position[2] += (-absf(sinf(o->Timer)) * 10.f + 10.f) * FPS_ANIMATION_FACTOR; // Fix Con R?ng Lorencia
	}
	o->Timer += 0.10f * FPS_ANIMATION_FACTOR; // C?p nh?t Timer theo FPS
}
...
void RenderLinkObject(float x, float y, float z, CHARACTER* c, PART_t* f, int Type, int Level, int Option1, bool Link, bool Translate, int RenderType, bool bRightHandItem)
{
	...
	if (Link)
	{
		...
		if (bRightHandItem == false && !(Type >= MODEL_SHIELD && Type < MODEL_SHIELD + MAX_ITEM_INDEX) && Type != MODEL_BOW + 20)
		{
			...
			Vector(145.f, 0.f, 180.f, vNewAngle);
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
	...
	if (g_pOption->m_Shadows != 1 && g_pOption->m_NShadows == 1 && gMapManager.WorldActive != WD_10HEAVEN && gMapManager.InHellas() == FALSE && !g_Direction.m_CKanturu.IsMayaScene())
	{
		switch (Type)        // ???? ??
		{
		...
		default:
			...
			if (Type >= MODEL_SWORD + 0 && Type <= MODEL_SWORD + 511) {
				b->RenderBodyShadow();
			}
			if (Type >= MODEL_SHIELD + 0 && Type <= MODEL_SHIELD + 511) {
				b->RenderBodyShadow();
			}
			if (Type >= MODEL_BOW + 0 && Type <= MODEL_BOW + 511) {
				b->RenderBodyShadow();
			}
			if (Type >= MODEL_STAFF + 0 && Type <= MODEL_STAFF + 511) {
				b->RenderBodyShadow();
			}
			if (Type >= MODEL_HELPER + 0 && Type <= MODEL_HELPER + 511) {
				b->RenderBodyShadow();
			}
			break;
		}
...
void RenderCharacter(CHARACTER* c, OBJECT* o, int Select)
{
	...
	rRenderLayOut characterBatch(o);
#endif
		...
		)
	{
		if (o->Alpha >= 0.3f)
		{
			...
			//	vPos[2] += 100.f;
			//	float size = 12.7f;
			//	CreateSprite(BITMAP_LIGHT, vPos, 16.0f, vLight, o, 0.5f);
			//	float fLumi;
			//	fLumi = sinf(WorldTime * 0.05f) * 0.4f + 0.9f;
			//	Vector(fLumi * 0.3f, fLumi * 0.7f, fLumi * 1.0f, vLight);
			//	CreateSprite(BITMAP_LIGHT, vPos, 8.0f, vLight, o);
			//	Vector(0.0f, 0.5f, 0.8f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_RING_OF_GRADATION, o->Position[0], o->Position[1], size, size, vLight, -WorldTime * 0.01f);
			//	Vector(1.0f, 1.0f, 1.f, vLight);
			//	fLumi = sinf(WorldTime * 0.0015f) * 0.3f + 0.5f;
			//	Vector(fLumi * 0.4f, fLumi * 0.7f, fLumi * 1.0f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_SUMMON_IMPACT, o->Position[0], o->Position[1], size, size, vLight, WorldTime * 0.01f);
			//	Vector(0.2f, 0.5f, 0.8f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_GM_AURORA, o->Position[0], o->Position[1], 7.5f, 7.5f, vLight, -WorldTime * 0.01f);
			//	DisableAlphaBlend();
			//	o->HiddenMesh = -1;
			//}
			//
			//if (c->MonsterIndex == 711)
			//{
			//	EnableAlphaBlend();
			//	vec3_t Position, Light;
			//	VectorCopy(o->Position, Position);
			...
			//	float fLumi;
			//	fLumi = sinf(WorldTime * 0.05f) * 0.4f + 0.9f;
			//	Vector(fLumi * 1.0f, fLumi * 0.4f, fLumi * 0.3f, vLight);
			//	CreateSprite(BITMAP_LIGHT, vPos, 8.0f, vLight, o);
			//	Vector(0.8f, 0.2f, 0.1f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_RING_OF_GRADATION, o->Position[0], o->Position[1], size, size, vLight, -WorldTime * 0.01f);
			//	Vector(1.0f, 1.0f, 1.f, vLight);
			//	fLumi = sinf(WorldTime * 0.0015f) * 0.3f + 0.5f;
			//	Vector(fLumi * 1.0f, fLumi * 0.4f, fLumi * 0.3f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_SUMMON_IMPACT, o->Position[0], o->Position[1], size, size, vLight, WorldTime * 0.01f);
			//	Vector(0.8f, 0.3f, 0.2f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_GM_AURORA, o->Position[0], o->Position[1], 7.5f, 7.5f, vLight, -WorldTime * 0.01f);
			//	DisableAlphaBlend();
			//	o->HiddenMesh = -1;
			//}
			//
			//if (c->MonsterIndex == 709)
			//{
			//	EnableAlphaBlend();
			//	vec3_t Position, Light;
			//	VectorCopy(o->Position, Position);
			...
			//	float size = 4.5f;
			//
			//	Vector(0.65f, 0.5f, 0.65f, vLight);
			//	CreateSprite(BITMAP_LIGHT, vPos, 13.0f, vLight, o, 0.5f);
			//
			//	float fLumi;
			//	fLumi = sinf(WorldTime * 0.05f) * 0.4f + 0.9f;
			//	Vector(fLumi * 0.65f, fLumi * 0.55f, fLumi * 0.8f, vLight);
			//	CreateSprite(BITMAP_LIGHT, vPos, 6.5f, vLight, o);
			//
			//	Vector(0.4f, 0.35f, 0.85f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_RING_OF_GRADATION, o->Position[0], o->Position[1], size, size, vLight, -WorldTime * 0.01f);
			//
			//	Vector(1.0f, 1.0f, 1.f, vLight);
			//	fLumi = sinf(WorldTime * 0.0015f) * 0.3f + 0.5f;
			//	Vector(fLumi * 0.7f, fLumi * 0.55f, fLumi * 0.9f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_SUMMON_IMPACT, o->Position[0], o->Position[1], size, size, vLight, WorldTime * 0.01f);
			//
			//	Vector(0.5f, 0.4f, 0.8f, vLight);
			//	RenderTerrainAlphaBitmap(BITMAP_GM_AURORA, o->Position[0], o->Position[1], 3.0f, 3.0f, vLight, -WorldTime * 0.01f);
			//
			//	DisableAlphaBlend();
			//	o->HiddenMesh = -1;
			//}
	...
	if (c->MonsterIndex == 38 || c->MonsterIndex == 43 || c->MonsterIndex == 52 || c->MonsterIndex == 59 || c->MonsterIndex == 67 || (78 <= c->MonsterIndex && c->MonsterIndex <= 83) || (c->MonsterIndex >= 493 && c->MonsterIndex <= 502))
	{
		...
		if (c->MonsterIndex == 43 || (78 <= c->MonsterIndex && c->MonsterIndex <= 83) || (c->MonsterIndex >= 493 && c->MonsterIndex <= 502))
		{
			...
			//Fix, efeito dos Dourados
			RenderPartObjectBodyColor(&Models[o->Type], o, o->Type, o->Alpha, RENDER_METAL + 8 | RENDER_BRIGHT + 2, Bright - 20);
		}

		if (c->MonsterIndex == 67)
		{
			RenderPartObjectBodyColor(&Models[o->Type], o, o->Type, o->Alpha, RENDER_CHROME | RENDER_BRIGHT + 2 | RENDER_EXTRA, Bright);
		}
		else
		{
	...
	if (!Bind)
	{
		if (!g_isCharacterBuff(o, eBuff_Cloaking))
		{
			constexpr float CritDamageEffectInterval = 1900.0f;
			if (g_isCharacterBuff(o, eBuff_AddCriticalDamage) && o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && (c->LastAttackEffectTime < WorldTime - CritDamageEffectInterval))
			{
				c->LastAttackEffectTime = WorldTime;
...
void RenderCharactersClient()
{
	for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
	{
		...
		if (g_pOption->m_Players && o->Type == MODEL_PLAYER && c != Hero)
		{
			continue;
		}
		...
		if (o->Live)
		{
			if (o->Visible)
			{
				...
				if (gMapManager.WorldActive == WD_77NEW_LOGIN_SCENE)
				{
					...
					float fDistance = sqrtf(fDistance_x * fDistance_x + fDistance_y * fDistance_y);
					if (!TestFrustrum2D(o->Position[0] * 0.01f, o->Position[1] * 0.01f, -100.f) || fDistance > 3800.f)	// ÄÃ¸µ ÃÖÀûÈ­ °¡´ÉÇÑ ºÎºÐ¡Ù
						continue;
				}
#endif //PJH_NEW_SERVER_SELECT_MAP
...
void ChangeCharacterExt(int Key, BYTE* Equipment, CHARACTER* pCharacter, OBJECT* pHelper)
{
	...
	c->MiniatureMode = ((Equipment[16] & 0x10) != 0);

	if (c == Hero)
	{
		g_bMiniatureMode = c->MiniatureMode;
	}
	...
	if (SceneFlag == 4)
	{
		...
		if (lpCharList != 0)
		{
			...
			{
				if (gCustomWing.CheckCustomWingByItem(gItemManager->GetItem(12, lpCharList->WingIndex)))
				{
			...
			if (lpCharList->PetIndex > 0)
			{
				...
				if (gHelperSystem.CheckIsHelper(MODEL_HELPER + lpCharList->PetIndex))
				{
					...
					gHelperManager.CreateHelper(gItemManager->GetItem(13, lpCharList->PetIndex), gHelperSystem.GetHelperModel(c->Helper.Type), c->Object.Position, c, 0);
				}
			}
		}
	}
	else
	{
		...
		if (lpList != 0)
		{
			...
			{
				if (gCustomWing.CheckCustomWingByItem(gItemManager->GetItem(12, lpList->WingIndex)))
				{
			...
			if (lpList->PetIndex > 0)
			{
				...
				if (gHelperSystem.CheckIsHelper(MODEL_HELPER + lpList->PetIndex))
				{
					...
					gHelperManager.CreateHelper(gItemManager->GetItem(13, lpList->PetIndex), gHelperSystem.GetHelperModel(c->Helper.Type), c->Object.Position, c, 0);
				}
			}
		}
	}
	...
	if (c == Hero)
	{
		ITEM* pCustomHelper = &CharacterMachine->Equipment[EQUIPMENT_HELPER];
		int CustomHelperType = pCustomHelper->Type + MODEL_ITEM;

		if (pCustomHelper->Type != -1 && gHelperSystem.CheckIsHelper(CustomHelperType))
		{
			if (c->Helper.Type != CustomHelperType || !gHelperManager.CheckExistsHelper(c))
			{
				DeleteBug(&c->Object);
				ThePetProcess().DeletePet(c, c->Helper.Type, true);
				gHelperManager.DeleteHelper(c);

				c->Helper.Type = CustomHelperType;
				gHelperManager.CreateHelper(pCustomHelper->Type, gHelperSystem.GetHelperModel(c->Helper.Type), c->Object.Position, c, 0);
			}
		}
	}
	...
	if (o->Type == MODEL_PLAYER)
	{
		...
		if (gMapManager.InChaosCastle() == false)
		{
			...
			if (w->Type != -1 && g_pOption->m_Wings == 0)
			{
                ...
                if (w->Type == MODEL_WING + 40
#ifdef PBG_ADD_NEWCHAR_MONK_ITEM
                    || w->Type == MODEL_WING + 50
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
                    || gCustomWing.CheckCustomWingByModelType(w->Type - MODEL_ITEM, 1))
                {
                    w->LinkBone = 19;
                ...
                else
                {
                    if (g_pOption->m_DefaultItem)
                    {
                        if (gCharacterManager.GetBaseClass(c->Class) == CLASS_WIZARD)
                        {
                            RenderLinkObject(0.f, 0.f, 15.f, c, w, (WINGSM), false, false, false, bTranslate);
                        }
                        if (gCharacterManager.GetBaseClass(c->Class) == CLASS_KNIGHT)
                        {
                            RenderLinkObject(0.f, 0.f, 15.f, c, w, (WINGBK), false, false, false, bTranslate);
                        }
                        if (gCharacterManager.GetBaseClass(c->Class) == CLASS_ELF)
                        {
                            RenderLinkObject(0.f, 0.f, 15.f, c, w, (WINGELF), false, false, false, bTranslate);
                        }
                        if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK)
                        {
                            RenderLinkObject(0.f, 0.f, 15.f, c, w, (WINGMG), false, false, false, bTranslate);
                        }
                        if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD)
                        {
                            RenderLinkObject(0.f, 0.f, 15.f, c, w, (WINGDL), false, false, false, bTranslate);
                        }
                        if (gCharacterManager.GetBaseClass(c->Class) == CLASS_SUMMONER)
                        {
                            RenderLinkObject(0.f, 0.f, 15.f, c, w, (WINGSUM), false, false, false, bTranslate);
                        }
                    }
                    else {
