...
void WebzenScene(HDC hDC)
{
	...
	{
		LoadBitmapA("Interface\\Loading\\KMUA2_Bar.jpg", 31122, 0x2601, 0x2901, 1, 0);
		LoadBitmapA("Interface\\Loading\\KMUA2_Loading.tga", 531123, 0x2601, 0x2901, 1, 0);
		LoadBitmapA("Interface\\Loading\\KMUA2_BarFrame.tga", 531124, 0x2601, 0x2901, 1, 0);

		int min = 1, max = (gProtect->m_MainInfo.LoadingImageCount > 15) ? 15 : gProtect->m_MainInfo.LoadingImageCount;
...
bool NewRenderCharacterScene(HDC hDC)
{
...
#ifdef PJH_NEW_SERVER_SELECT_MAP
#ifdef NEM_MOD_CAM_POS_AND_ANGLE_ON_CHAR
	Vector(8898.0f, 18513.0f, 550.0f, pos);	// SEASON 2/3
#else // !NEM_MOD_CAM_POS_AND_ANGLE_ON_CHAR
	Vector(9758.0f, 18913.0f, 675.0f, pos);	// SEASON 5/6
#endif // NEM_MOD_CAM_POS_AND_ANGLE_ON_CHAR
#else //PJH_NEW_SERVER_SELECT_MAP
	Vector(23566.0f, 14085.0f, 395.0f, pos);	// SEASON 4
#endif //PJH_NEW_SERVER_SELECT_MAP
	...
	RenderBitmap(BITMAP_LOG_IN + 10, (float)Width, (float)25, (float)Width, (float)Height, 0.f, 0.f);
	EndBitmap();
#endif //PJH_NEW_SERVER_SELECT_MAP
	Height = 480;
...
void CreateLogInScene()
{
	...
	{
		if (g_pLoginVideo == nullptr)
		{
			g_pLoginVideo = new CVideoTextureMF();
		}

		wchar_t videoPath[] = L"Data\\Custom\\Movie\\LoginMovie.mp4";

		if (!g_pLoginVideo->Open(videoPath, true))
		{
			delete g_pLoginVideo;
			g_pLoginVideo = nullptr;
		}
	}
...
bool NewRenderLogInScene(HDC hDC)
{
	...
	Height = 320;
	RenderBitmap(BITMAP_LOG_IN + 9, (float)0, (float)25, (float)Width, (float)Height, 0.f, 0.f);
	RenderBitmap(BITMAP_LOG_IN + 10, (float)Width - 1, (float)25, (float)Width + 1, (float)Height, 0.f, 0.f);
	EndBitmap();
#endif //PJH_NEW_SERVER_SELECT_MAP
	...
	if (CCameraMove::GetInstancePtr()->IsTourMode())
	{
		...
		RenderBitmap(BITMAP_CHROME + 3, 0.0f, 0.0f, GetWindowsX, GetWindowsY, 800.f * fScale, 600.f * fScale, (800.f) / 1024.f - 800.f * fScale * 2, (600.f) / 1024.f - 600.f * fScale * 2);
	...
	g_pRenderText->RenderText(setPosMidRight(335) - Size.cx * 640 / WindowWidth, GetWindowsY - Size.cy * 640 / WindowWidth - 10, Text);
	if (m_Resolution < 2)
	{
		strcpy(Text, GlobalText[455]);
		g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), Text, lstrlen(Text), &Size);
		g_pRenderText->RenderText(setPosMidRight(335), GetWindowsY - Size.cy * 640 / WindowWidth - 10, Text);
	}
	sprintf(Text, GlobalText[456], m_ExeVersion);
	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), Text, lstrlen(Text), &Size);
	g_pRenderText->RenderText(0, GetWindowsY - Size.cy * 640 / WindowWidth - 10, Text);
...
bool MoveMainCamera()
{
	...
	else
	{
		...
		else
		{
			...
			static bool wasInMainScene = false;
			static int faseZoom = 0;
			static float alvoAtual = 0.0f;
			...
			if (SceneFlag == MAIN_SCENE)
			{
				if (!wasInMainScene)
				{
					...
					wasInMainScene = true;
					faseZoom = 1;
					alvoAtual = CameraDistanceTarget * 1.2f;
				}

				float speed = 0.25f * FPS_ANIMATION_FACTOR;

				if (faseZoom == 1)
				{
					CameraDistance += (alvoAtual - CameraDistance) * speed;

					if (CameraDistance >= alvoAtual - 50.0f)
					{
						faseZoom = 2;
						alvoAtual = CameraDistanceTarget;
					}
				}
				else if (faseZoom == 2)
				{
					...
					if (CameraDistance <= alvoAtual + 10.0f)
					{
						CameraDistance = alvoAtual;
					}
				}
			}
			else
			{
				wasInMainScene = false;
				faseZoom = 0;
...
void MoveMainScene()
{
	...
	{
		if (MouseY >= (int)(int)(GetWindowsY - 51))
			MouseOnWindow = true;
	...
	// ===================================================================
	// AVISO DE TEMPO DE JOGO A CADA HORA
	// ===================================================================
	static DWORD dwPlayTimeStart = 0;
	static int iLastSentHour = 0;

	if (dwPlayTimeStart == 0)
	{
		dwPlayTimeStart = timeGetTime();
	}

	DWORD dwCurrentTime = timeGetTime();
	DWORD dwPlayedMs = dwCurrentTime - dwPlayTimeStart;

	int iCurrentHour = (int)(dwPlayedMs / 3600000);
...
#include <string>
#include <XboxInput.h>
float GetFpsFromConfig()
{
	char szCurrentDir[256];
	GetCurrentDirectory(256, szCurrentDir);
	std::string szIniFilePath = std::string(szCurrentDir) + (szCurrentDir[strlen(szCurrentDir) - 1] == '\\' ? "Data\\Custom\\config.ini" : "\\Data\\Custom\\config.ini");
	...
	if (file.is_open()) {
		while (std::getline(file, line)) {
			if (line.find("FpsLimit") != std::string::npos) {
...
void MainScene(HDC hDC)
{
	...
	if (gProtect->m_MainInfo.m_UseIntegratedAntiHack == 0 && gProtect->m_MainInfo.m_AntiBypassDLL == 1)
	{
		if (strlen(gProtect->m_MainInfo.PluginName1) > 0 || strlen(gProtect->m_MainInfo.PluginName2) > 0 ||
			...
			strlen(gProtect->m_MainInfo.PluginName5) > 0)
		{
	...
	if (XboxInput::IsPressX())
	{
		MouseLButtonPush = true;
		MouseLButton = true;
	}
	else if (XboxInput::IsReleaseX())
	{
		MouseLButton = false;
	}

	if (XboxInput::IsPressA())
	{
		MouseRButtonPush = true;
		MouseRButton = true;
	}
	else if (XboxInput::IsReleaseA())
	{
		MouseRButton = false;
	}
	...
	//unicode::_sprintf(szMousePos, "MousePos : X:%d | Y:%d |Z:%d", MouseX, MouseY, MouseLButtonPush);
	...
	if (SceneFlag == MAIN_SCENE)
	{
		...
		//static DWORD dwLastScriptCheck = 0;
		//
		//DWORD dwCurrentTime = GetTickCount();
		//
		//if (dwCurrentTime - dwLastScriptCheck >= 5000)
		//{
		//	dwLastScriptCheck = dwCurrentTime;
		//
		//	const char* FilesToCheck[] = {
		//		"Data\\Lua\\Manager\\Interfaces\\GetHardwareID.lua",
		//
		//		"Data\\Lua\\Manager\\Central\\Configs\\AccountLock_Config.lua",
		//
		//		"Data\\Lua\\Manager\\Central\\Interface\\AccountLock.lua",
		//	};
		//
		//	int nFiles = sizeof(FilesToCheck) / sizeof(FilesToCheck[0]);
		//
		//	for (int i = 0; i < nFiles; i++)
		//	{
		//		WIN32_FIND_DATA FindFileData;
		//		HANDLE hFind = FindFirstFile(FilesToCheck[i], &FindFileData);
		//
