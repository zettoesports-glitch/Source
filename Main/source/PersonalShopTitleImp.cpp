...
{
	float Width = FrameBeginOpengl(), Height = 480;
...
void CPersonalShopTitleImp::CalculateBooleanPos(IN CHARACTER* pPlayer, IN const SIZE& size, OUT POINT& pos)
{
	...
	// [2026-08-30] Tag da Personal Shop acima da cabeca (HP bar / nome do char).
	Vector(pObject->Position[0],pObject->Position[1],pObject->Position[2]+pObject->BoundingBoxMax[2]+150.f,posTemp);
	...
	// [2026-08-30] Adapta dinamicamente ao tamanho do texto (mesmo padrao do balao de chat).
	pos.y = (ptFloating.y*(int)WindowHeight/480) - (int)size.cy;
}
