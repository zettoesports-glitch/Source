...
void SEASON3B::CNewUIGuildInfoWindow::SetButton()
{
...
bool SEASON3B::CNewUIGuildInfoWindow::Check_Mouse(int mx,int my)
{
	...
	if(m_nCurrentTab == 0)
	{
		if(SEASON3B::CheckMouseIn(m_Pos.x+163, m_Pos.y+262+m_Loc, 18, 33+m_Loc) == true && m_EventState == EVENT_NONE)
		{
			...
			if(m_BackUp == 0)
			{
				m_BackUp	= m_Pos.y + (262 + (MouseY - 262));
			}
			return false;
		}
	}
	else if(m_nCurrentTab == 1)
	{
		if((MouseX > m_Pos.x+166 && MouseX < (m_Pos.x+181) && MouseY > m_Pos.y+ 125 + m_Loc && MouseY < (m_Pos.y+175 + m_Loc)) && m_EventState == EVENT_NONE)
		{
			m_EventState = EVENT_SCROLL_BTN_DOWN;
			if(m_BackUp == 0)
				m_BackUp	= m_Pos.y + (125 + (MouseY - 125));
			return false;
		}
	}
	return true;
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Text()
{
	...
	if( m_nCurrentTab == 0 ) 
	{
		...
		g_pRenderText->SetTextColor(255, 255, 255, 255);
		g_pRenderText->RenderText(m_Pos.x + 15, m_Pos.y + 249, Text, 120, 0, RT3_SORT_LEFT);
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Guild_History()
{
	...
	RenderImage(IMAGE_GUILDINFO_SCROLL_TOP, m_Pos.x + 167, m_Pos.y + 262, 7, 3);
		
	for(int i=0; i<7; i++)
	{
		RenderImage(IMAGE_GUILDINFO_SCROLL_MIDDLE, m_Pos.x + 167, m_Pos.y + 261 + (float)(i * 10 + 4), 7, 15);
	}
	
	RenderImage(IMAGE_GUILDINFO_SCROLL_BOTTOM, m_Pos.x+167, m_Pos.y + 338, 7, 3);
	...
	RenderImage(IMAGE_GUILDINFO_SCROLLBAR_ON, m_Pos.x+163, m_Pos.y + 262 + m_Loc, 15, 30);
...
void SEASON3B::CNewUIGuildInfoWindow::RenderScrollBar()
{
	...
	RenderImage(IMAGE_GUILDINFO_SCROLL_TOP, m_Pos.x+170, m_Pos.y + 125, 7, 3);
	
	for(int i=0; i<22; i++)
		RenderImage(IMAGE_GUILDINFO_SCROLL_MIDDLE, m_Pos.x+170, m_Pos.y + 124+(float)(i*10+4), 7, 15);
	
	RenderImage(IMAGE_GUILDINFO_SCROLL_BOTTOM, m_Pos.x+170, m_Pos.y + 352, 7, 3);
	...
	if (m_EventState == EVENT_SCROLL_BTN_DOWN) 
		...
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	RenderImage(IMAGE_GUILDINFO_SCROLLBAR_ON, m_Pos.x+166, m_Pos.y + 125 + m_Loc, 15, 30);
