...
//	//	g_ConsoleDebug->Write(MCD_ERROR, "LoadItemModel %d %s %s %d", index, path, name, value);
//}
...
//	gLoadData.AccessModel(MODEL_HELPER+132, "Data\\Item\\", "goldenmaple_cham");
	...
	for (std::map<int, CUSTOM_ITEM_INFO>::iterator it = gItemManager->m_CustomItemInfo.begin(); it != gItemManager->m_CustomItemInfo.end(); it++)
	{
		...
		if (ItemInfo->Index != -1)
		{
			gLoadData.AccessModel(ItemInfo->ItemIndex, ((ItemInfo->ItemIndex >= gItemManager->GetItemModel(7, 0) && ItemInfo->ItemIndex < gItemManager->GetItemModel(12, 0)) ? "Data\\Player\\" : "Data\\Item\\"), ItemInfo->ModelName);
		}
	}
	...
	//	gLoadData.AccessModel((info.ItemIndex + MODEL_ITEM), "Data\\Item\\", info.ModelName);
	//}
}
...
//	wsprintf(path, "Data\\%s", folder);
//
//	gLoadData.OpenTexture(index, folder, 0x2901, 0x2600, 0x01);
//	//g_ConsoleDebug->Write(MCD_ERROR, "Loadtexture %d [%s]", index, folder);
//}
...
//---------------------------------------------------------------------------------
	...
	for (std::map<int, CUSTOM_ITEM_INFO>::iterator it = gItemManager->m_CustomItemInfo.begin(); it != gItemManager->m_CustomItemInfo.end(); it++)
	{
		...
		if (ItemInfo->Index != -1)
		{
			gLoadData.OpenTexture(ItemInfo->ItemIndex, ((ItemInfo->ItemIndex >= gItemManager->GetItemModel(7, 0) && ItemInfo->ItemIndex < gItemManager->GetItemModel(12, 0)) ? "Player\\" : "Item\\"));
		}
	}
	...
	//	gLoadData.OpenTexture((info.ItemIndex + MODEL_ITEM), "Item\\");
	//}
}
...
void OpenBasicData(HDC hDC)
{
	...
	rUIMng.RenderTitleSceneUI(hDC, 0, 22);
...
//	LoadBitmap("Monster\\gigantiscorn_R.jpg", BITMAP_GIGANTIS_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	...
	LoadBitmap("Interface\\hp_bar_bg.tga", BITMAP_HP_BAR_BG, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmap("Interface\\hp_bar_fill.tga", BITMAP_HP_BAR_FILL, GL_LINEAR, GL_CLAMP_TO_EDGE);
	LoadBitmap("Interface\\hp_bar_frame.tga", BITMAP_HP_BAR_FRAME, GL_LINEAR, GL_CLAMP_TO_EDGE);
	...
	LoadWaveFile(SOUND_PETEFFECT01,		"Data\\Sound\\effectpet.wav",1);
