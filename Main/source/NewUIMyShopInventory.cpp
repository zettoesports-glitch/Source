...
bool SEASON3B::CNewUIMyShopInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if (false == m_pNewInventoryCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, m_Pos.x + 16, m_Pos.y + 90, 8, 4, this->GetLayerDepth() + 0.1f))
	{
	...
	m_Button[MYSHOPINVENTORY_OPEN].ChangeButtonInfo(m_Pos.x + 13, m_Pos.y + 183, 36, 29);
	...
	m_Button[MYSHOPINVENTORY_CLOSE].ChangeButtonInfo(m_Pos.x + 53, m_Pos.y + 183, 36, 29);
...
bool SEASON3B::CNewUIMyShopInventory::UpdateMouseEvent()
{
	...
	for (int i = 1; i < MYSHOPINVENTORY_MAXBUTTONCOUNT; ++i)
	{
...
{
	RenderImage(IMAGE_MYSHOPINVENTORY_BACK, m_Pos.x, m_Pos.y, 190.f, 214.5f);
	RenderImage(IMAGE_MYSHOPINVENTORY_TOP, m_Pos.x, m_Pos.y, 190.f, 64.f);
	RenderImage(IMAGE_MYSHOPINVENTORY_LEFT, m_Pos.x, m_Pos.y + 64, 21.f, 150.f);
	RenderImage(IMAGE_MYSHOPINVENTORY_RIGHT, m_Pos.x + 190 - 21, m_Pos.y + 64, 21.f, 150.f);
	RenderImage(IMAGE_MYSHOPINVENTORY_BOTTOM, m_Pos.x, m_Pos.y + 214 - 45, 190.f, 45.f);
...
void SEASON3B::CNewUIMyShopInventory::RenderTextInfo()
{
	...
	if (m_EnablePersonalShop)
	{
		RenderText(GlobalText[1103], m_Pos.x + 37, m_Pos.y + 195, 190, 0, RGBA(215, 138, 0, 255), 0x00000000, RT3_SORT_CENTER, g_hFontBold);
	}
