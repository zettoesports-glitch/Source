...
bool SEASON3B::CNewUIMiniMap::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	LoadBitmap("Interface\\mini_map_ui_monster.tga", IMAGE_MINIMAP_INTERFACE + 7, GL_LINEAR);

	m_BtnExit.ChangeButtonImgState(true, IMAGE_MINIMAP_INTERFACE + 6, false);
	m_BtnExit.ChangeButtonInfo(GetWindowsX - 27, 3, 85, 85);
...
bool SEASON3B::CNewUIMiniMap::Render()
{
	...
	EnableAlphaTest();
	RenderColor(0, 0, GetWindowsX, ((m_Resolution > 2) ? GetWindowsY : GetWindowsY - 51), 0.85f, 1);
	DisableAlphaBlend();
	EnableAlphaTest();
	...
	for (i = 0; i < MAX_MINI_MAP_DATA; i++)
	{
		if (m_Mini_Map_Data[i].Kind > 0)
		{
			...
			else if (m_Mini_Map_Data[i].Kind == 2) // portais
			{
			...
			else if (m_Mini_Map_Data[i].Kind == 3) // spots
			{
				RenderPointRotate(IMAGE_MINIMAP_INTERFACE + 7, Tx1, Ty1, NpcWidth, NpcWidth, m_Lenth[m_MiniPos].x - Tx, m_Lenth[m_MiniPos].y - Ty, m_Lenth[m_MiniPos].x, m_Lenth[m_MiniPos].y, Rot, Rot_Loc, 17.5f / 32.f, 17.5f / 32.f, i);
			}
		}
		else
			break;
	...
	float Ch_wid = 12;
	RenderImage(IMAGE_MINIMAP_INTERFACE + 3, setPosMidRight(325), setPosMidDown(230), Ch_wid, Ch_wid, 0.f, 0.f, 17.5f / 32.f, 17.5f / 32.f);
	...
	for (i = 0; i < MAX_MINI_MAP_DATA; i++)
	{
		if (m_Mini_Map_Data[i].Kind > 0)
		{
			if (mx > m_Btn_Loc[i][0] && mx < (m_Btn_Loc[i][0] + m_Btn_Loc[i][2]) && my > m_Btn_Loc[i][1] && my < (m_Btn_Loc[i][1] + m_Btn_Loc[i][3]))
			{
				...
				Fontsize.cy = Fontsize.cy / ((float)WindowHeight / GetWindowsY);
