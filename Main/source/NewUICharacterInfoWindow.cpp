...
void SEASON3B::CNewUICharacterInfoWindow::MoveWindow(int x, int y)
{
	SetPos(x, y);
...
void SEASON3B::CNewUICharacterInfoWindow::SetButtonInfo()
{
	...
	m_BtnQuest.ChangeButtonInfo( m_Pos.x + 50, m_Pos.y + 398, 36, 29 );
	...
	m_BtnPet.ChangeButtonInfo( m_Pos.x + 87, m_Pos.y + 398, 36, 29 );
	...
	m_BtnMasterLevel.ChangeButtonInfo(m_Pos.x + 124, m_Pos.y + 398, 36, 29);
...
bool SEASON3B::CNewUICharacterInfoWindow::BtnProcess()
{
	...
	if (m_BtnMasterLevel.UpdateMouseEvent() == true)
	{
		...
		//if (ServerCode >= 20 && ServerCode <= 39)
		//{
			if (gCharacterManager.IsMasterLevel(Hero->Class) == true && gCharacterManager.GetCharacterClass(Hero->Class) != CLASS_DIMENSIONMASTER
...
void SEASON3B::CNewUICharacterInfoWindow::RenderTableTexts()
{
	...
	{
		unicode::_sprintf(strLevel, GlobalText[200], CharacterAttribute->Level, CharacterAttribute->ViewResets);
