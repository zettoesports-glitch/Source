...
void ProcessDamageCastle( LPPRECEIVE_ATTACK Data)
{
	...
	else
	{
		...
		{
			if (c->MonsterIndex != 275 && rand_fps_check(2))
			{
				SetPlayerShock(c, Damage);
			}
		}
...
void CalcAddPositionFix(OBJECT* o, float x, float y, float z, vec3_t Position)
{
	float Matrix[3][4];
...
void ReceiveTradeYourInventoryDelete( BYTE *ReceiveBuffer )
{
	...
	Console.Write(1, "0x37 [ReceiveTradeYourInventoryDelete(%d)]", Data->Value);
...
void ReceiveChaosMixRateInfo(BYTE* ReceiveBuffer)
{
	LPCHAOSMIX_RATE Data = (LPCHAOSMIX_RATE)ReceiveBuffer;

	gChaosMix->MixRate = Data->rate;

	gChaosMix->MixMoney = Data->money;

	gChaosMix->MixTax = Data->tax;
}

void ReceiveAddPointNewProtocol(BYTE* ReceiveBuffer)
{
	...
	Console.Write(1, "[0xE0] Resets: %d", Data->ViewReset);
...
void ReceiveMoveToEventMatchResult2( BYTE *ReceiveBuffer )
{
	...
	case 8:
		...
		break;
	case 9:
		SEASON3B::CreateOkMessageBox(GlobalText[3322]);
		break;
	case 10:
		SEASON3B::CreateOkMessageBox(GlobalText[3323]);
		break;
	}
}
...
{
	Console.Write(1, "[0x%02x]", HeadCode);
	...
	case 0xF3:     			
		{
			...
			case 0x52:
				...
				break;
			case 0xE0:
				ReceiveAddPointNewProtocol(ReceiveBuffer);
				break;
			case 0xE2:
	...
	case 0x87:
		...
		break;
	case 0x88:
		ReceiveChaosMixRateInfo(ReceiveBuffer);
		break;
	case 0x8E:
		{
			LPPHEADER_DEFAULT_SUBCODE Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
