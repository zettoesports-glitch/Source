...
void CUIMng::CreateTitleSceneUI()
{
    ...
    else
    {
        ...
        m_asprTitle[UIM_TS_MU].Create(216, 138, BITMAP_TITLE + 2, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, _fScaleXTemp, _fScaleXTemp);
...
float RenderNumber2D2(float x, float y, int Num, float Width, float Height)
{
    char Text[32];
    itoa(Num, Text, 10);
    ...
    RenderBitmap(531126, x + 5, y, Width + 3, Height, 0.f, 0.f, 9.f / 16.f, 14.f / 16.f, 1, 1, 0.0);
    return x;
}
...
void CUIMng::CreateLoginScene()
{
    ...
    m_ServerSelWin.SetPosition((rInput.GetScreenWidth() - m_ServerSelWin.GetWidth()) / 2, (rInput.GetScreenHeight() - m_ServerSelWin.GetHeight()) / 2);

    m_LoginWin.Create();
    m_WinList.AddHead(&m_LoginWin);
    m_LoginWin.SetPosition((rInput.GetScreenWidth() - m_LoginWin.GetWidth()) / 2, (rInput.GetScreenHeight() - m_LoginWin.GetHeight()) * 2 / 3);
