...
bool SEASON3B::CNewUIMasterLevel::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_BtnExit.ChangeButtonInfo(m_Pos.x + 591 + GetCenterX(640), setPosMidDown(m_Pos.y + 390), 36, 29);
...
bool SEASON3B::CNewUIMasterLevel::Render()
{
	...
	m_Pos.x = (GetWindowsX - m_Width.x) / 2;

	m_Pos.y = ((GetWindowsY - 51) - m_Width.y) / 2;
...
void SEASON3B::CNewUIMasterLevel::Render_Scroll()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		RenderImage(IMAGE_MASTER_SCROLLBAR_ON, m_Pos.x + Scroll_Loc[i][0], setPosMidDown(Scroll_Loc[i][1] + m_Loc[i]), 15, 30);
	}
}
	...
	for (int Skill = 0; Skill < 4; Skill++)
	{
		...
		for (int sy = StartY; sy < (Wid + StartY); sy++)
		{
			...
			for (int sx = 0; sx < m_MasterLevel[GetJob + Skill].Width; sx++)
			{
				...
				RenderImage(IMAGE_MASTER_INTERFACE + 1, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, setPosMidDown(70 + ((SKILL_ICON_HEIGHT + 14) * nsy)), 35, 34, 0.f, 0.f, nx / 64.f, ny / 64.f, Color);
				RenderImage(IMAGE_MASTER_SKILL_ICON, Loc[Skill] + 5 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, setPosMidDown(75 + ((SKILL_ICON_HEIGHT + 14) * nsy)), (float)15, (float)21, (20.f / 256.f) * (Skill_Icon % 12), ((28.f / 256.f) * ((Skill_Icon / 12) + 4)), 20.f / 256.f, 27.5f / 256.f, Color);
				if (Skill_Num > 0)
				{
					...
					g_pRenderText->RenderText(Loc[Skill] + 31 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, setPosMidDown(95 + ((SKILL_ICON_HEIGHT + 14) * nsy)), lpszStr, 0, 0, RT3_WRITE_CENTER);
				...
				if (Dir == 1)
					RenderImage(IMAGE_MASTER_INTERFACE + 2, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) + 9 + PosX, setPosMidDown(70 + ((SKILL_ICON_HEIGHT + 14) * nsy) - 12), 8, 12, 0.f, 18.f / 32.f, 14.8f / 16.f, -(18.f / 32.f));
				if (Dir == 2)
					RenderImage(IMAGE_MASTER_INTERFACE + 3, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) + 27 + PosX, setPosMidDown(70 + ((SKILL_ICON_HEIGHT + 14) * nsy) + 11), 12, 8, 0.f, 0.f, 18.f / 32.f, 14.8f / 16.f);
				if (Dir == 3)
					RenderImage(IMAGE_MASTER_INTERFACE + 2, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) + 9 + PosX, setPosMidDown(70 + ((SKILL_ICON_HEIGHT + 14) * nsy) + 31), 8, 12, 0.f, 0.f, 14.8f / 16.f, 18.f / 32.f);
				if (Dir == 4)
					RenderImage(IMAGE_MASTER_INTERFACE + 3, Loc[Skill] + ((SKILL_ICON_WIDTH + 19) * sx) - 12 + PosX, setPosMidDown(70 + ((SKILL_ICON_HEIGHT + 14) * nsy) + 11), 12, 8, 18.f / 32.f, 0.f, -(18.f / 32.f), 14.8f / 16.f);
		...
		for (int sy = StartY; sy < (Wid + StartY); sy++)
		{
			...
			for (int sx = 0; sx < m_MasterLevel[GetJob + Skill].Width; sx++)
			{
				...
				if (!(SEASON3B::IsPress(VK_LBUTTON)))
				{
					...
					if (CheckMouseIn(Loc[Skill] + 5 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, setPosMidDown(75 + ((SKILL_ICON_HEIGHT + 14) * nsy)), 15, 21))
					{
						...
						RenderTipTextList(Loc[Skill] + 5 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, setPosMidDown(75 + ((SKILL_ICON_HEIGHT + 14) * nsy)), ib, 0, RT3_SORT_CENTER, FALSE);
					}
				}
			}
		}
	...
	for (int Skill = 0; Skill < 4; Skill++)
	{
		...
		for (int sy = StartY; sy < (Wid + StartY); sy++)
		{
			...
			for (int sx = 0; sx < m_MasterLevel[GetJob + Skill].Width; sx++)
			{
				...
				if (SEASON3B::IsPress(VK_LBUTTON) && CheckMouseIn(Loc[Skill] + 5 + ((SKILL_ICON_WIDTH + 19) * sx) + PosX, setPosMidDown(75 + ((SKILL_ICON_HEIGHT + 14) * nsy)), 15, 21))
				{
