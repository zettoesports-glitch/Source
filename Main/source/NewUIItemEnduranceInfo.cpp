...
void SEASON3B::CNewUIItemEnduranceInfo::RenderLeft()
{
	...
	if (RenderPCPoints(m_UIStartPos.x, iNextPosY))
	{
...
void SEASON3B::CNewUIItemEnduranceInfo::RenderFpsPing()
{
	unicode::t_char szDebugText[128];

	const float fps = (float)FPS_AVG;
	const int ping = gsteady_clock->get_ping_time();
	...
	const float x = 12.f;
	const float y = pos_botton(458.f);
	...
	g_pRenderText->RenderText((int)x, (int)y, szDebugText, 0, 0, RT3_SORT_LEFT);

	unicode::_sprintf(szDebugText, "  Ping:%dms", ping);
	g_pRenderText->SetTextColor(pr, pg, pb, 255);
	g_pRenderText->RenderText((int)(x + 58.f), (int)y, szDebugText, 0, 0, RT3_SORT_LEFT);
