...
void WorkdMovimentMiniature(OBJECT* ObjectStruct, float MaxPos, int Type)
{
	vec3_t OwnerPos;
	float Angle;
	ObjectStruct->Position[2] = ObjectStruct->Owner->Position[2];
	Vector(ObjectStruct->Owner->Position[0] - ObjectStruct->Position[0],
		ObjectStruct->Owner->Position[1] - ObjectStruct->Position[1],
		ObjectStruct->Owner->Position[2] - ObjectStruct->Position[2], OwnerPos);
	...
	ObjectStruct->Position[0] += VecPos[0];
	ObjectStruct->Position[1] += VecPos[1];
	ObjectStruct->Position[2] += VecPos[2];
	...
	float normalScale = 0.9f;

	ObjectStruct->Scale += (targetScale - ObjectStruct->Scale) * 0.08f * FPS_ANIMATION_FACTOR;

	if (ObjectStruct->Scale < targetScale + 0.01f)
		ObjectStruct->Scale = targetScale;

	ObjectStruct->Velocity = 0.5f;
}
	...
	if ( o->Live )
	{
        ...
        case MODEL_DARK_HORSE:
			if ((TerrainWall[TERRAIN_INDEX_REPEAT((int)floor(o->Owner->Position[0] / 100.0f), (int)floor(o->Owner->Position[1] / 100.0f))] & 1) == 1)
			{
				if (gProtect && gProtect->m_MainInfo.m_MountMiniSafeZone != 0 && g_pOption->m_MiniSafeZone)
				{
					WorkdMovimentMiniature(o, 180.f, 16);
				}
				else
				{
					o->Alpha = 0.0f;
		...
		case MODEL_UNICON:
			if ((TerrainWall[TERRAIN_INDEX_REPEAT((int)floor(o->Owner->Position[0] / 100.0f), (int)floor(o->Owner->Position[1] / 100.0f))] & 1) == 1)
			{
				if (gProtect && gProtect->m_MainInfo.m_MountMiniSafeZone != 0 && g_pOption->m_MiniSafeZone)
				{
					WorkdMovimentMiniature(o, 180.f, 4);
				}
				else
				{
					o->Alpha = 0.0f;
	...
	if(o->Live)
	{
		...
		if(o->Visible)
		{
			...
			else if (SceneFlag == CHARACTER_SCENE)
			{
				...
				if (o->Type == MODEL_FENRIR_BLACK || o->Type == MODEL_FENRIR_BLUE || o->Type == MODEL_FENRIR_RED || o->Type == MODEL_FENRIR_GOLD)
				{
					o->Scale = 0.85f;
				}
				...
				if (o->Type == MODEL_PEGASUS || o->Type == MODEL_UNICON)
				{
					o->Scale = 1.0f;
				}
