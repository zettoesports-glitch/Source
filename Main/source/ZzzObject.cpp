...
//			b->RenderBody(RENDER_TEXTURE,o->Alpha,o->BlendMesh,o->BlendMeshLight,o->BlendMeshTexCoordU,o->BlendMeshTexCoordV);
		...
		else if(o->Type == MODEL_FENRIR_BLACK || o->Type == MODEL_FENRIR_BLUE || o->Type == MODEL_FENRIR_RED || o->Type == MODEL_FENRIR_GOLD)
		{
			...
			{
				CreateEffect(MODEL_FENRIR_THUNDER, rPosition, o->Angle, vLight, 0, o);
				CreateEffect(MODEL_FENRIR_THUNDER, rPosition, o->Angle, vLight, 0, o);
			}
			else
			{
...
//					Vector(0.7f,0.5f,0.8f,b->BodyLight);
			...
			{
				if (o->CurrentAction == MONSTER01_DIE || (WorldTime - o->InitialSceneFrame) < 1000)
				{
			...
			if (o->Visible)
			{
				...
				if (o->Type == MODEL_POTION + 15)
				{
					...
					for (int j = 1; j < Count; j++)
					{
						...
						AngleMatrix(Angle, Matrix);
						/*if (g_pNewUISystem->GetUI_NewOptionWindow()->OnOffGrap[g_pNewUISystem->GetUI_NewOptionWindow()->eBMDZen])VectorRotate(p, Matrix, Position);*/
...
void RenderPartObjectBody(BMD *b,OBJECT *o,int Type,float Alpha,int RenderType)
{
	...
	else if (Type == MODEL_WING + 36) //Fix Asa lvl 3 de BM
	{
		...
		if (timeGetTime() - s_dwLastUpdate >= 50) 
		{
			s_iTexAni++;
			if (s_iTexAni > 15)
				s_iTexAni = 0;

			s_dwLastUpdate = timeGetTime();
		}

		float fU = (s_iTexAni / 4) * 0.25f;
...
{	
	...
	if (gDisableExcellent.CheckDisableExcellent(Type) == 1)
	{
		Level = 0;
	}
		...
		if (p->m_pCloth[0])
		{
			if (iCloth > 0)
			{
				for (int i = 0; i < p->m_byNumCloth; ++i)
				{
					...
					if (!pCloth->Move2(0.005f, 5)) //fix FPS 60
					{
