...
bool CNewUIMixInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if (false == m_pNewInventoryCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, x + 15, y + 110, 8, 4, this->GetLayerDepth() + 0.1f))
	{
		SAFE_DELETE(m_pNewInventoryCtrl);
		return false;
	}

	// posição recebida do sistema (ORIGINAL DO CLIENT)
	SetPos(x, y);
...
{
	//m_MoveWindow.Create(m_Pos.x, m_Pos.y, INVENTORY_WIDTH, INVENTORY_HEIGHT);
...
bool CNewUIMixInventory::UpdateMouseEvent()
{
	...
	// área de itens (não inclui cabeçalho arrastável)
	if (CheckMouseIn(m_Pos.x, m_Pos.y + 64, INVENTORY_WIDTH, INVENTORY_HEIGHT - 64))
	{
...
int SEASON3B::CNewUIMixInventory::InventoryFindItemSlot(int cMouseX, int cMouseY)
{
	return m_pNewInventoryCtrl->FindItemptIndex(cMouseX, cMouseY);
}

ITEM* SEASON3B::CNewUIMixInventory::InventoryFindItem(int cMouseX, int cMouseY)
{
	return m_pNewInventoryCtrl->FindItemAtPt(cMouseX, cMouseX);
}