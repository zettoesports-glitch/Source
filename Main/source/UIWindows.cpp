...
DWORD CUIWindowMgr::AddWindow(int iWindowType, int iPos_x, int iPos_y, const char* pszTitle, DWORD dwParentID, int iOption)
{
	...
	if (!(iOption & UIADDWND_FORCEPOSITION))
	{
		for (m_WindowMapIter = m_WindowMap.begin(); m_WindowMapIter != m_WindowMap.end(); ++m_WindowMapIter)
		{
			...
			{
				if (iPos_x + pbw->GetWidth() + 20 <= GetWindowsX) iPos_x += 20;
				if (iPos_y + pbw->GetHeight() + 20 <= GetWindowsY) iPos_y += 20;
				if (iPos_x + pbw->GetWidth() + 20 > GetWindowsX && iPos_y + pbw->GetHeight() + 20 > GetWindowsY)
				{
					if (iPos_y % 10 == 9)
					{
						delete pbw;
...
void CUIWindowMgr::HideAllWindow(BOOL bHide, BOOL bMainClose)
{
	...
	if (bHide == FALSE)
	{
		...
		{
			OpenMainWnd(GetWindowsX - 250, setPosDown(432 - 170));
		}
...
BOOL CUIBaseWindow::DoMouseAction()
{
	...
	if (GetState() == UISTATE_MOVE)
	{
		if (MouseLButton == true)
		{
			...
			else if (m_iPos_x + m_iWidth + MouseX - m_iMouseClickPos_x > GetWindowsX) m_iPos_x = GetWindowsX - m_iWidth;
			...
			else if (m_iPos_y + m_iHeight + MouseY - m_iMouseClickPos_y > GetWindowsY)
			{
				m_iPos_y = GetWindowsY - m_iHeight;
			}
			else 
			{
	...
	else if (GetState() == UISTATE_RESIZE)
	{
		if (MouseLButton == true)
		{
			if (m_iResizeDir == 135)
			{
				...
				if (m_iPos_x + m_iWidth + MouseX - m_iMouseClickPos_x > GetWindowsX) m_iWidth = GetWindowsX - m_iPos_x;
				...
				if (m_iPos_y + m_iHeight + MouseY - m_iMouseClickPos_y > GetWindowsY) m_iHeight = GetWindowsY - m_iPos_y;
	...
	case UI_MESSAGE_BTNLCLICK:
		{
			...
			case 1:	
				if (m_iShowType == 1)
				{
					...
					if (m_iPos_x + m_iWidth > GetWindowsX) m_iPos_x = GetWindowsX - m_iWidth;
					Refresh();
				}
