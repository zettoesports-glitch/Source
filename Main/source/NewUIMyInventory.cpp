...
SEASON3B::CNewUIMyInventory::CNewUIMyInventory()
{
	...
	m_Pos.x = m_Pos.y = 0;
	m_PosTemp.x = m_PosTemp.y = 0;
	m_PosOffset.x = m_PosOffset.y = 0;
...
bool SEASON3B::CNewUIMyInventory::Create(CNewUIManager* pNewUIMng, CNewUI3DRenderMng* pNewUI3DRenderMng, int x, int y)
{
	...
	m_pNewUI3DRenderMng->Add3DRenderObj(this, m_fLayerDepth + 0.1f);

	m_pNewInventoryCtrl = new CNewUIInventoryCtrl;
	if (false == m_pNewInventoryCtrl->Create(m_pNewUI3DRenderMng, g_pNewItemMng, this, x + 15, y + 200, 8, 8, m_fLayerDepth + 0.1f))
	{
...
void SEASON3B::CNewUIMyInventory::SetPos(int x, int y)
{
	...
	m_BtnExit.SetPos(m_Pos.x + 13, m_Pos.y + 398);
	m_BtnRepair.SetPos(m_Pos.x + 50, m_Pos.y + 398);
	m_BtnMyShop.SetPos(m_Pos.x + 87, m_Pos.y + 398);
	m_BtnPrev.SetPos(m_Pos.x + 128, m_Pos.y + 400);
	m_BtnNext.SetPos(m_Pos.x + 156, m_Pos.y + 400);
}
...
bool SEASON3B::CNewUIMyInventory::UpdateMouseEvent()
{
	...
	//			if (m_pNewInventoryCtrl)
	//			{
	//				m_pNewUI3DRenderMng->Remove3DRenderObj(m_pNewInventoryCtrl);
	//				m_pNewUI3DRenderMng->Add3DRenderObj(m_pNewInventoryCtrl, m_fLayerDepth + 0.1f);
	//			}
	//		}
	//	}
	//	else
...
#ifndef SOCKET_ACTIVATE
	if (CheckMouseIn(m_Pos.x + INVENTORY_WIDTH * 0.4f, m_Pos.y + 20, INVENTORY_WIDTH * 0.2f, 15) == true)
#else
...
bool SEASON3B::CNewUIMyInventory::Update()
{
	...
	if (m_WindowNext == 1)
	{
		...
		{
			m_iPointedSlot = -1;
			for (int i = 12; i < MAX_EQUIPMENT_INDEX; i++)
			{
				if (CheckMouseIn(m_EquipmentSlots[i].x + 1, m_EquipmentSlots[i].y,
					m_EquipmentSlots[i].width - 4, m_EquipmentSlots[i].height - 4))
				{
...
bool SEASON3B::CNewUIMyInventory::Render()
{
	...
	if (m_WindowNext == 1)
	{
		gItemBank->Render(m_Pos.x, m_Pos.y);
	}
...
void SEASON3B::CNewUIMyInventory::SetButtonInfo()
{
	...
	m_BtnRepair.ChangeButtonInfo(m_Pos.x + 50, m_Pos.y + 398, 36, 29);
	...
	m_BtnMyShop.ChangeButtonInfo(m_Pos.x + 87, m_Pos.y + 398, 36, 29);
	m_BtnMyShop.ChangeToolTipText(GlobalText[1125], true);

	m_BtnPrev.ChangeButtonImgState(true, 32340, true, false, false);
	m_BtnPrev.ChangeButtonInfo(m_Pos.x + 128, m_Pos.y + 400, 20, 23);
	m_BtnPrev.ChangeToolTipText(GlobalText[1012], true);

	m_BtnNext.ChangeButtonImgState(true, 32341, true, false, false);
	m_BtnNext.ChangeButtonInfo(m_Pos.x + 156, m_Pos.y + 400, 20, 23);
	m_BtnNext.ChangeToolTipText(GlobalText[1013], true);
}
