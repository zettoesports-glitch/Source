				 ...
				 WORD SkillIndex,WORD SkillSerialNum,int iChaIndex, const float* vPriorColor, short int sTargetindex)
{
	for(int i=0;i<MAX_JOINTS;i++)
	{
		...
		if(!o->Live )
		{
			...
			case BITMAP_2LINE_GHOST: //Skill Birds
			{
				...
				if (o->SubType == 0)
				{
					if (g_pOption->m_DisableSkill != 1)
					{
						if (g_pOption->m_EffectSprite != 1)
						{
							...
							{
								o->RenderType = RENDER_TYPE_ALPHA_BLEND_MINUS; //Goc
							}
			...
			case BITMAP_JOINT_SPIRIT2:
				if (o->SubType == 0 || g_pOption->m_NSkillCollor != true)
				{
					o->RenderType = RENDER_TYPE_ALPHA_BLEND_MINUS;
				}
				switch (o->SubType)
...
void MoveJoint( JOINT *o, int iIndex)
{
	...
	case BITMAP_JOINT_HEALING:
		...
		break;
	case BITMAP_2LINE_GHOST: //Fix Effect Qua
	{
		if (o->SubType == 0)
		{
	...
	case BITMAP_FLASH:
		if (o->SubType <= 3 || o->SubType == 5)
		{
			...
			Vector(0.f, -o->Velocity * FPS_ANIMATION_FACTOR, 0.f, p);
			VectorRotate(p, Matrix, Position);
			VectorAddScaled(o->Position, Position, o->Position, FPS_ANIMATION_FACTOR); // D?u 66666666666666666666666666666666666666666666666
	...
	o->LifeTime -= FPS_ANIMATION_FACTOR; // l?i
	if(o->LifeTime < 0)
	{
		o->Live = false;
	}
