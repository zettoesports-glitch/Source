...
void SEASON3B::CNewUIMainFrameWindow::RenderFrame()
{
	...
	width = 256.f; height = 51.f; x = 0.f; y = 480.f - height;

	SEASON3B::RenderImage(IMAGE_MENU_1, x + frame_cx, frame_cy, width, height);
	//
	x = x + frame_cx - 70.28048780487805;

	RenderBitmap(IMAGE_CUSTOM_ID, x, frame_cy, 70.28048780487805, height, 0.0, 0.0, 0.8785000443, 0.6409999728, 1, 1, 0.0);
	//
	width = 128.f; x = 256.f; 
	SEASON3B::RenderImage(IMAGE_MENU_2, x + frame_cx, frame_cy, width, height);
	//
	width = 256.f; x = 256.f + 128.f;
	SEASON3B::RenderImage(IMAGE_MENU_3, x + frame_cx, frame_cy, width, height);
	//
	RenderBitmap(IMAGE_CUSTOM_ID, x + frame_cx + width, frame_cy, 70.28048780487805, height, 0.8785000443, 0.0, -0.8785000443, 0.6409999728, 1, 1, 0.0);

	if(g_pSkillList->IsSkillListUp() == true)
	{
		width = 160.f; height = 40.f; x = 222.f;
		SEASON3B::RenderImage(IMAGE_MENU_2_1, x + frame_cx, frame_cy, width, height);
	}
}
...
void SEASON3B::CNewUIMainFrameWindow::RenderGuageAG()
{
	...
	x = 256 + 128 + 36 + frame_cx; y = frame_cy - 49.f;
	...
	SEASON3B::RenderNumber(x+10, frame_cy - 20.f, (int)dwSkillMana, 0.8f);
	...
	{	
		...
		RenderTipText((int)x-20, frame_cy - 68.f, strTipText);
    }
}
...
void SEASON3B::CNewUIMainFrameWindow::RenderGuageSD()
{
	...
	{	
		...
		RenderTipText((int)x-20, (int)frame_cy - 68.f, strTipText);
    }
}
...
void SEASON3B::CNewUIMainFrameWindow::RenderExperience()
{
	...
	float frame_cx = GetCenterX(640);
	float frame_cy = GetWindowsY - 480.f;
	...
	if (gCharacterManager.IsMasterLevelExpCheck(CharacterAttribute->Class) == true)
	{
		...
		__int64 iData_Master = (((__int64)9 + (__int64)iTotalLevel) * (__int64)iTotalLevel * (__int64)iTotalLevel * (__int64)10) + (((__int64)9 + (__int64)iTOverLevel) * (__int64)iTOverLevel * (__int64)iTOverLevel * (__int64)1000);
		...
		x = 2.f + frame_cx; y = 473.f; width = fProgress * 629.f; height = 4.f;

		if (Master_Level_Data.nMLevel == 400)
		{
			fExpBarNum = 9;
			width = 629.f;
		}

		RenderBitmap(IMAGE_MASTER_GAUGE_BAR, x, y + frame_cy, width, height, 0.f, 0.f, 6.f/8.f, 4.f/4.f);

		int iExp = (int)fExpBarNum;
		x = 635.f + frame_cx; y = 469.f + frame_cy;
		SEASON3B::RenderNumber(x, y, iExp);

		x = 2.f + frame_cx; y = 473.f + frame_cy; width = 629.f; height = 4.f;
		if(SEASON3B::CheckMouseIn(x, y, width, height) == true)
		{	
			char strTipText[256];
	
			sprintf(strTipText, GlobalText[1748], dwExperience, dwNexExperience);
			RenderTipText(280 + frame_cx, 418 + frame_cy, strTipText);
		}
	}
	else
	{
		...
		x = 2.f + frame_cx; y = 473.f + frame_cy; width = fProgress * 629.f; height = 4.f;

		if (CharacterAttribute->Level == 400)
		{
			fExpBarNum = 9;
			width = 629.f;
		}
...
void SEASON3B::CNewUIMainFrameWindow::RenderCharInfoButton()
{
	...
	if (m_bButtonBlink)
	{
		if (!(g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_QUEST_PROGRESS_ETC) || g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHARACTER)))
		{
			...
			m_BtnChaInfo.ChangeImgIndex(IMAGE_MENU_BTN_CHAINFO, 3);
		}
	}
	else
	{
...
void SEASON3B::CNewUIMainFrameWindow::RenderFriendButtonState()
{
	...
	{
		RenderImage(IMAGE_MENU_BTN_FRIEND, 489 + (30 * 3) + GetCenterX(640), GetWindowsY - 51, 30, 41, 0.0f, 123.f);
	}
	else
	{
		RenderImage(IMAGE_MENU_BTN_FRIEND, 489 + (30 * 3) + GetCenterX(640), GetWindowsY - 51, 30, 41, 0.0f, 41.f);
	}
#else //defined PBG_ADD_INGAMESHOP_UI_MAINFRAME
	if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_FRIEND) == true)
	{
		RenderImage(IMAGE_MENU_BTN_FRIEND, 488 + 76 + GetCenterX(640), GetWindowsY - 51, 38, 42, 0.0f, 126.f);
	}
	else
	{
		RenderImage(IMAGE_MENU_BTN_FRIEND, 488 + 76 + GetCenterX(640), GetWindowsY - 51, 38, 42, 0.0f, 42.f);
	}
#endif//defined PBG_ADD_INGAMESHOP_UI_MAINFRAME
}
...
{
	if (XboxInput::IsPressRT())
		g_pSkillList->UseHotKey(1);
	if (XboxInput::IsPressRB())
		g_pSkillList->UseHotKey(2);
	if (XboxInput::IsPressLB())
		g_pSkillList->UseHotKey(3);
	if (XboxInput::IsDownLT())
		g_pSkillList->UseHotKey(4);
...
bool SEASON3B::CNewUIItemHotKey::UpdateKeyEvent()
{
	...
	static int iLastDPadPressTime = 0;
	static int iDPadPressCount = 0;
	...
	if (SEASON3B::IsPress(VK_SHIFT))
	{
		if (currentTime - iLastShiftPressTime <= DOUBLE_PRESS_DELAY)
		{
			...
			if (iShiftPressCount >= 2)
			{
				...
				g_pChatListBox->AddText("", bAutoKeyMode ? "System [2X Shift] AutoPotion [ON]" : "System [2X Shift] AutoPotion [OFF]", SEASON3B::TYPE_SYSTEM_MESSAGE);
				iShiftPressCount = 0;
			}
		}
		else
	...
	if (XboxInput::IsPressDPadDown())
	{
		if (currentTime - iLastDPadPressTime <= DOUBLE_PRESS_DELAY)
		{
			iDPadPressCount++;
			if (iDPadPressCount >= 2)
			{
				bAutoKeyMode = !bAutoKeyMode;
				g_pChatListBox->AddText(
					"",
		...
		iLastDPadPressTime = currentTime;
	}
...
void SEASON3B::CNewUIItemHotKey::RenderItems()
{
	...
	for(int i=0; i<HOTKEY_COUNT; ++i)
	{
		...
		if(iIndex != -1)
		{
			...
			if(pItem)
			{
				x = GetCenterX(640) + 10 + (i*38); y = GetWindowsY - 37.f; width = 20; height = 20;
...
void SEASON3B::CNewUIItemHotKey::RenderItemCount()
{
	...
	for(int i=0; i<HOTKEY_COUNT; ++i)
	{
		...
		if(iCount > 0)
		{
			x = GetCenterX(640) + 30 + (i*38); y = GetWindowsY - 23.f; width = 8; height = 9;
...
bool SEASON3B::CNewUISkillList::UpdateMouseEvent()
{
	...
	int PosY = GetWindowsY;
	
	x = 385.f + PosX; y = PosY - 49.f; width = 32.f; height = 38.f;
	...
	x = 222.f + PosX; y = PosY - 49.f; width = 32.f*5.f; height = 38.f;
	if(m_EventState == EVENT_NONE && MouseLButtonPush == false 
		&& SEASON3B::CheckMouseIn(x, y, width, height) == true)
	{
	...
	x = 190.f + PosX; y = PosY - 49.f; width = 32.f; height = 38.f;
	...
	x = 385.f + PosX; y = PosY - 90; width = 32; height = 38;
	float fOrigX = 385.f + PosX;
	...
	if(Hero->m_pPet != NULL)
	{
		x = 353.f + PosX; y = PosY - 128; width = 32; height = 38;
...
void SEASON3B::CNewUISkillList::RenderCurrentSkillAndHotSkillList()
{
	...
	if(bySkillNumber > 0)
	{
		...
		x = 190 + frame_cx; y = frame_cy - 49.f ; width = 32; height = 38;
		for(i=0; i<5; ++i)
		{
			x += width;
		...
		x = 392 + frame_cx; y = frame_cy - 43; width = 20; height = 28;
...
bool SEASON3B::CNewUISkillList::Render()
{
	...
	if(bySkillNumber > 0)
	{
		if(m_bSkillList == true)
		{
			...
			x = 385 + frame_cx; y = frame_cy - 90; width = 32; height = 38;
			float fOrigX = 385.f + frame_cx;
            int iSkillType  = 0;
			int iSkillCount = 0;
...
void SEASON3B::CNewUISkillList::RenderPetSkill()
{
	...
	float frame_cx = GetCenterX(640);
	float frame_cy = GetWindowsY;
	x = 353.f + frame_cx; y = frame_cy - 128; width = 32; height = 38;
