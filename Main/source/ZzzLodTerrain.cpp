...
static  const float g_fGrassFrustumRange = -300.f;
...
int OpenTerrainAttribute(char* FileName)
{
	...
	{
		iSize = EncBytes - 4;
		std::vector<BYTE> FileBuffer(iSize, 0);
		fread(FileBuffer.data(), 1, iSize, fp);
		fclose(fp);

		MuFileDecrypt2(FileBuffer);
		byBuffer = new unsigned char[iSize];
		memcpy(byBuffer, FileBuffer.data(), iSize);
	}
	else
	{
		fseek(fp, 0, SEEK_SET);
		...
		iSize = MapFileDecrypt(NULL, EncData, EncBytes);
		byBuffer = new unsigned char[iSize];
...
int OpenTerrainMapping(char* FileName)
{
	...
	else
	{
		...
		FileBuffer = std::vector<BYTE>(DataBytes, 0);
		MapFileDecrypt(FileBuffer.data(), EncData, EncBytes);
		delete[] EncData;
	}

	int DataPtr = 0;
	BYTE Version = FileBuffer[DataPtr++];
	int iMapNumber = (int)FileBuffer[DataPtr++];
	memcpy(TerrainMappingLayer1, &FileBuffer[DataPtr], 256 * 256); DataPtr += 256 * 256;
	memcpy(TerrainMappingLayer2, &FileBuffer[DataPtr], 256 * 256); DataPtr += 256 * 256;
	for (int i = 0; i < TERRAIN_SIZE * TERRAIN_SIZE; i++)
	{
		BYTE Alpha;
		Alpha = FileBuffer[DataPtr++];
		TerrainMappingAlpha[i] = (float)Alpha / 255.f;
	}

	// Custom terrain files may use 255 or a missing extended tile as a sentinel.
	// Normalize those values before rendering so no invalid OpenGL texture is bound.
	int textureUseCount[30] = { 0 };
	for (int i = 0; i < TERRAIN_SIZE * TERRAIN_SIZE; ++i)
	{
		if (TerrainMappingLayer1[i] < 30)
		{
			++textureUseCount[TerrainMappingLayer1[i]];
		}
	}

	int fallbackTexture = 0;
	for (int i = 1; i < 30; ++i)
	{
		if (textureUseCount[i] > textureUseCount[fallbackTexture])
		{
			fallbackTexture = i;
		}
	}
	...
	for (int i = 0; i < TERRAIN_SIZE * TERRAIN_SIZE; ++i)
	{
		if (TerrainMappingLayer1[i] >= 30)
		{
			TerrainMappingLayer1[i] = fallbackTexture;
		}

		if (TerrainMappingLayer2[i] >= 30)
		{
...
static bool OpenTerrainLightBmp(char* FileName, float* BufferFloat)
{
	char OzbFileName[256];
	...
	strcpy(OzbFileName, "Data\\");
	strcat(OzbFileName, NewFileName);
	strcat(OzbFileName, "OZB");

	FILE* fp = fopen(OzbFileName, "rb");
	if (fp == NULL)
	{
		return false;
	}
	...
	if (iBytes < 58 + (TERRAIN_SIZE * TERRAIN_SIZE * 3))
	{
		fclose(fp);
		return false;
	}
	...
	if (header.bfType != 0x4D42 || bmiHeader.biWidth != TERRAIN_SIZE || abs(bmiHeader.biHeight) != TERRAIN_SIZE || bmiHeader.biBitCount != 24)
	{
		delete[] pbyData;
		return false;
	}

	DWORD dwPixelPos = 4 + header.bfOffBits;
	for (int y = 0; y < TERRAIN_SIZE; y++)
	{
		int srcY = (bmiHeader.biHeight > 0) ? (TERRAIN_SIZE - 1 - y) : y;
		BYTE* src = &pbyData[dwPixelPos + srcY * TERRAIN_SIZE * 3];
		for (int x = 0; x < TERRAIN_SIZE; x++)
		{
			int index = (y * TERRAIN_SIZE + x) * 3;
			BufferFloat[index] = (float)src[x * 3 + 2] / 255.f;
			BufferFloat[index + 1] = (float)src[x * 3 + 1] / 255.f;
			BufferFloat[index + 2] = (float)src[x * 3] / 255.f;
		}
	}
...
{
	if (OpenTerrainLightBmp(FileName, &TerrainLight[0][0]) == false)
	{
...
{
	if (gCustomWorld.IsTerrainHeightExtMap(iWorld))
	{
		return true;
	}
...
static int GetLoadedTerrainTexture(int texture, int fallbackTexture)
{
	if (texture >= 0 && texture < 30 && Bitmaps.FindTexture(BITMAP_MAPTILE + texture) != NULL)
	{
		return texture;
	}

	if (fallbackTexture >= 0 && fallbackTexture < 30 && Bitmaps.FindTexture(BITMAP_MAPTILE + fallbackTexture) != NULL)
	{
	...
	for (int i = 0; i < 30; ++i)
	{
		if (Bitmaps.FindTexture(BITMAP_MAPTILE + i) != NULL)
		{
			return i;
		}
	}
...
static bool HasLoadedTerrainGrassTexture()
{
	for (int i = 0; i < 30; ++i)
	{
		if (Bitmaps.FindTexture(BITMAP_MAPGRASS + i) != NULL)
		{
			return true;
		}
	}
...
static bool TerrainTileHasGrass(int x, int y)
{
	if (x < 0 || y < 0 || x >= TERRAIN_SIZE_MASK || y >= TERRAIN_SIZE_MASK)
	{
		return false;
	}

	int index = TERRAIN_INDEX(x, y);
	if (TerrainMappingAlpha[index] > 0.f)
	{
		return false;
	}

	int texture = TerrainMappingLayer1[index];
	return (texture >= 0 && texture < 30 && Bitmaps.FindTexture(BITMAP_MAPGRASS + texture) != NULL);
}

static bool TerrainBlockHasGrass(int xi, int yi)
{
	for (int y = yi; y < yi + 4 && y < TERRAIN_SIZE_MASK; ++y)
	{
		for (int x = xi; x < xi + 4 && x < TERRAIN_SIZE_MASK; ++x)
		{
			if (TerrainTileHasGrass(x, y))
			{
				return true;
			}
		}
...
static bool TerrainBlockHasGround(int xi, int yi)
{
	for (int y = yi; y < yi + 4 && y < TERRAIN_SIZE_MASK; ++y)
	{
		for (int x = xi; x < xi + 4 && x < TERRAIN_SIZE_MASK; ++x)
		{
			if ((TerrainWall[TERRAIN_INDEX(x, y)] & TW_NOGROUND) != TW_NOGROUND)
			{
				return true;
			}
		}
...
void RenderTerrainFace(float xf, float yf, int xi, int yi, float lodf)
{
	...
	if (TerrainFlag != TERRAIN_MAP_GRASS)
	{
		...
		Texture = GetLoadedTerrainTexture(Texture, TerrainMappingLayer1[TerrainIndex1]);
		if (Texture >= 0)
		{
...
void CreateFrustrum2D(vec3_t Position)
{
		...
		else
		{
			...
			case 0:
...
#else
					Width = (float)GetScreenWidths() / 640.f * 10.0f * 0.575995f;
#endif
				}
				...
				if (SceneFlag == LOG_IN_SCENE)
				{
					...
					CameraViewNear = 2400.f * 17.0f * 0.5f;
					CameraViewTarget = 2400.f * 17.0f * 0.48f;
	...
	else if (gMapManager.WorldActive == WD_73NEW_LOGIN_SCENE || gMapManager.WorldActive == WD_77NEW_LOGIN_SCENE)
	{
...
bool TestFrustrum2D(float x, float y, float Range)
{
	...
	if (CameraZoom > 0)
	{
		adjustedRange -= (15.0f * CameraZoom);
	}

	if (AngleY3D < 0)
	{
		adjustedRange += (15.0f * AngleY3D);
	}
...
void CreateFrustrum(float Aspect, vec3_t position)
{
	...
	float Height = Width * 123.f / 124.f;
	vec3_t Temp[5];
...
void RenderTerrainFrustrum(bool EditFlag, float FrustumRange)
{
	int     xi;
	int     minX = FrustrumBoundMinX;
	int     minY = FrustrumBoundMinY;
	int     maxX = FrustrumBoundMaxX;
	int     maxY = FrustrumBoundMaxY;

	if (TerrainFlag == TERRAIN_MAP_GRASS)
	{
		const int grassBoundsMargin = 8;
		minX = max(0, minX - grassBoundsMargin);
		minY = max(0, minY - grassBoundsMargin);
		maxX = min(TERRAIN_SIZE_MASK - 4, maxX + grassBoundsMargin);
		maxY = min(TERRAIN_SIZE_MASK - 4, maxY + grassBoundsMargin);
	}
	...
	for (; yi <= maxY; yi += 4, yf += 4.f)
	{
		xi = minX;
		xf = (float)xi;
		for (; xi <= maxX; xi += 4, xf += 4.f)
		{
			if (TerrainFlag == TERRAIN_MAP_GRASS)
			{
				if (!TerrainBlockHasGrass(xi, yi))
				{
					continue;
				}
			}
			else if (!EditFlag && !TerrainBlockHasGround(xi, yi))
			{
				continue;
			}

			if (TerrainFlag == TERRAIN_MAP_GRASS || TestFrustrum2D(xf + 2.f, yf + 2.f, FrustumRange) || CameraTopViewEnable)
			{
#ifdef PJH_NEW_SERVER_SELECT_MAP
...
void RenderTerrain(bool EditFlag)
{
	...
	if (!EditFlag && g_pOption->m_Terrain)
	{
		DisableDepthTest();
		EnableCullFace();
		RenderPointers();
	...
	RenderTerrainFrustrum(EditFlag, g_fFrustumRange);
	//  
	...
	if (!EditFlag)
	{
		...
		// [2026-08-30] Mato alto do terreno (cartoes verticais TileGrass) desligado.
		// Continua o chao com textura de relva e as hortas 3D (Grass.bmd).
		//if (TerrainGrassEnable && HasLoadedTerrainGrassTexture() && gMapManager.WorldActive != WD_7ATLANSE && !IsDoppelGanger3())
		//{
		//	glAlphaFunc(GL_GREATER, 0.0f);
		//	TerrainFlag = TERRAIN_MAP_GRASS;
		//	RenderTerrainFrustrum(EditFlag, g_fGrassFrustumRange);
