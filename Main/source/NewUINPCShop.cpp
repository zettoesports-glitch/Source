...
bool SEASON3B::CNewUINPCShop::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if(false == m_pNewInventoryCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, x+15, y+50, 8, 15, this->GetLayerDepth() + 0.1f))
	{
	...
	//m_MoveWindow.Create(x, y, NPCSHOP_POS_X, NPCSHOP_POS_Y);
...
void SEASON3B::CNewUINPCShop::RenderTexts()
{
	if (CPcPoints::Instance().IsPCRoomPointShopMode())
	{
		...
		const unicode::t_char* c_apszGlobalText[c_nNoticeMax] = { GlobalText[2327], GlobalText[2328], GlobalText[2329] };
