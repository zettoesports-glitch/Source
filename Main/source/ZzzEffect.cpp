...
{
	if (o->Type == MODEL_SUMMONER_SUMMON_LAGUL)
	{
		for (int i = 48; i <= 53; ++i)
		{
	...
	else if (o->Type == MODEL_SKILL_WHEEL1)
	{
		if (o->Owner)
		{
	...
	if (to != NULL && to->Live && to->Visible)
	{
		...
		if (Range <= 100.f)
		{
			...
			case BITMAP_ENERGY:
				...
				{
					CreateParticleFpsChecked(BITMAP_SPARK + 1, o->Position, o->Angle, Light, 20, 3.f);
				}
				CreateParticleFpsChecked(BITMAP_SPARK + 1, o->Position, o->Angle, Light, 0, 6.f);
				break;
		...
		if (!o->Live)
		{
			...
			case MODEL_FIRE:
				...
				else if (o->SubType == 6)
				{
					o->LifeTime = 40;
					o->HiddenMesh = 1;
					o->Scale = (float)(rand() % 8 + 15) * 0.033f;
					o->PKKey = -1;
					o->Velocity = -10;
					o->Position[0] += (rand() % 100 + 200);
					o->Position[1] += (rand() % 100 - 50);
					o->Position[2] += (rand() % 300 + 500);
					Vector(0.f, 0.f, 1.f - rand() % 190, o->Direction);
					Vector(0.f, 20.f, 0.f, o->HeadAngle);

					CreateJoint(BITMAP_SMOKE, o->Position, o->Position, o->HeadAngle, 0, o, 5.f);
				}
				else if (o->SubType == 7)
				{
					o->LifeTime = 40;
			...
			case MODEL_BLIZZARD:
				if (o->SubType == 0 || o->SubType == 2)
				{
					...
					rangeX = 200; rangeY = 100; rangeZ = 400;
					o->Scale = 0.5f;
			...
			case MODEL_PIER_PART:
				...
				else if (o->SubType == 1)
				{
					...
					o->Alpha = (float)((20 - o->LifeTime) / 5.f);
					Vector(0.f, 0.f, 0.f, o->Direction);
					if (g_pOption->m_NSkillCollor != 0)
					{
						CreateParticle(BITMAP_FIRE + 2, o->Position, o->Angle, o->Light, 2.5f, 2.f, o);
					}
					else
					{
						CreateParticle(BITMAP_FIRE + 1, o->Position, o->Angle, o->Light, 0, 1.f, o);
			...
			case BITMAP_TARGET_POSITION_EFFECT2:
			{
				...
				{
					o->LifeTime = 30;
					// [2026-08-30] Visual S6: anel do destino ao andar (cursorpin02).
					o->Scale = 1.8f;
				}
			}
			break;
...
void MoveEffect(OBJECT* o, int iIndex)
{
	...
	case BITMAP_JOINT_THUNDER:
	{
		...
		if (g_pOption->m_NSkillCollor)
		{
			...
			if (rand() % 10 > 4)
			{
				if (o->SubType == 1)
				{
					CreateJoint(BITMAP_JOINT_THUNDER, vStartPosition, vPosition, o->Angle, 11, NULL, fScale);
				}
				else
				{
					CreateJoint(BITMAP_JOINT_THUNDER, vStartPosition, vPosition, o->Angle, 11, NULL, fScale);
			...
			//Efeito de Fumaça
			if (rand() % 4 == 0)
				CreateParticle(BITMAP_SMOKE, vPosition, o->Angle, vLight, 20, 2.8f);

			if (rand() % 4 == 0)
			{
				CreateEffect(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, vLight, 14);
			}

			if (o->LifeTime > 5)
			{
				Vector(1.f, 0.85f, 0.f, vLight);
				if (rand() % 5 == 0)
	...
	case MODEL_ALICE_DRAIN_LIFE:
	{
		...
		if (o->LifeTime <= 70 && o->LifeTime >= 66)
		{
			...
			for (int i = 0; i < iCnt2; i++)
			{
				...
				CreateJoint(BITMAP_DRAIN_LIFE_GHOST, vSourcePos, vTargetPos, o->Angle, 0, pSourceObj, 20.f, 0, 0, 0, -1, vLight);
			}
		}
	...
	case BITMAP_LIGHT:
		...
		else if (o->SubType == 1)
		{
			if (o->Owner != NULL && o->Owner->Live == true &&
				g_isCharacterBuff(o->Owner, eBuff_HpRecovery)
				|| g_isCharacterBuff(o->Owner, EFFECT_GREATER_LIFE_ENHANCED)
				|| g_isCharacterBuff(o->Owner, EFFECT_GREATER_LIFE_MASTERED)
				)
				o->LifeTime = 10;
			else
				o->LifeTime = 0;
	...
	case MODEL_MAGIC1:
		...
		break;
	case MODEL_SKILL_WHEEL1:
		CreateEffectFpsChecked(MODEL_SKILL_WHEEL2, o->Position, o->Angle, o->Light, (4) - (int)o->LifeTime, o->Owner, o->PKKey, o->Skill, o->Kind);
		break;
	case MODEL_SKILL_WHEEL2:
		switch (o->SubType)
		{
		...
		CreateParticleFpsChecked(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 93);
	...
	case MODEL_MAGIC2:
		...
		for (int j = 0;j < 4;j++)
			if (rand_fps_check(1))
				...
				{
					CreateParticle(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 2);
				}
		if (g_pOption->m_NSkillCollor != 1)
		{
			Vector(Luminosity * 0.3f, Luminosity * 0.6f, Luminosity, Light);
		...
		{
			Vector(Luminosity * 0.6f, Luminosity * 0.6f, Luminosity * 0.0f, Light);
		}
		...
		if (o->SubType == 2)
		{
			...
			if (rand_fps_check(1))
				...
				{
					CreateParticle(BITMAP_SMOKE, o->Position, o->Angle, Light, 13, (float)(rand() % 32 + 80) * 0.015f);
				}
	...
	case MODEL_FIRE:
		if (o->SubType == 0 || o->SubType == 2 || o->SubType == 4 || o->SubType == 6 || o->SubType == 7 || o->SubType == 8)
		{
			...
			if (success)
			{
				...
				if (o->SubType == 6 || o->SubType == 7 || o->SubType == 8)
				{
					...
					//
					//	Vector(0.1f, 0.1f, 0.1f, Light);
					//	CreateParticleFpsChecked(BITMAP_SMOKE+1, Position, o->Angle, Light, 0, (float)(rand() % 32 + 80) * 0);
					//}
		...
		else
		{
			...
			if (o->SubType == 6 || o->SubType == 7 || o->SubType == 8)
			{
				Vector(Luminosity * 0.1f, Luminosity * 1.f, Luminosity * 2.f, Light);
			}
			else
			{
				Vector(Luminosity * 1.f, Luminosity * 0.1f, Luminosity * 0.f, Light);
			...
			if (o->SubType == 6 || o->SubType == 7)
			{
				for (int j = 0; j < 2; j++)
				{
					Position[0] = o->Position[0] + rand() % 10 - 25;
					Position[1] = o->Position[1];
	...
	case MODEL_BLIZZARD:
		if (o->SubType == 0)
		{
			o->Position[0] = o->StartPosition[0] + sinf((rand() % 1000) * 0.01f) * 5.f;
			o->Position[1] = o->StartPosition[1] + sinf((rand() % 1000) * 0.01f) * 5.f;
			o->Position[2] += (o->Gravity) * FPS_ANIMATION_FACTOR;
			o->Gravity -= (rand() % 5) * FPS_ANIMATION_FACTOR;

			o->StartPosition[0] -= (10.f) * FPS_ANIMATION_FACTOR;

			//CreateParticleFpsChecked(BITMAP_SMOKE, o->Position, o->Angle, Light, 0, 0.5f);
			if (rand_fps_check(2))
			{
			...
			{
				CreateParticleFpsChecked(BITMAP_FIRE + 3, o->Position, o->Angle, Light, 7, o->Scale - 1);
			}
			...
			if (o->Position[2] < Height)
			{
				...
				{
					CreateParticleFpsChecked(BITMAP_SMOKE + 4, Position, o->Angle, Light, 11, (float)(rand() % 32 + 20) * 0.035f);
	...
	case BITMAP_SBUMB:
	{
		...
		if ((int)o->LifeTime == 15 
			...
			|| (int)o->LifeTime == 9 || (int)o->LifeTime == 6)
			...
			if (o->Visible)
			{
				...
				case BITMAP_LIGHT_MARKS:
				{
					...
					for (int i = 0; i < nBoneCount; ++i)
					{
						...
						//CreateSprite(o->Type, vPos, o->Scale * fScale[i], o->Light, o->Owner);  //Fix Buff Summoner
					}
				}
				break;
				case MODEL_SWELL_OF_MAGICPOWER_BUFF_EFF:
			...
			if (o->Visible)
			{
				...
				case BITMAP_FLAME:
					if (o->SubType != 3 && o->SubType != 6)
					{
						...
						Vector(Luminosity, Luminosity, Luminosity, Light);
						//RenderTerrainAlphaBitmap(BITMAP_FLAME, o->Position[0], o->Position[1], 0.001f, 0.001f, Light, -o->Angle[2]);
					}
					break;
				case BITMAP_LIGHTNING + 1:
					o->Scale += (0.2f) * FPS_ANIMATION_FACTOR;
