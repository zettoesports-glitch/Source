...
void SEASON3B::CNewUIPickedItem::SetPos(int x, int y)
{
...
void SEASON3B::CNewUIInventoryCtrl::Init()
{
	...
	m_bRepairMode = false;
	m_bCanPushItem = true;
	Vector(1.1f, 1.4f, 1.8f, m_afColorStateNormal);
	Vector(2.f, 0.0f, 0.0f, m_afColorStateWarning);
}
...
bool SEASON3B::CNewUIInventoryCtrl::Create(CNewUI3DRenderMng* pNew3DRenderMng, CNewUIItemMng* pNewItemMng,
	CNewUIObj* pOwner, int x, int y, int nColumn, int nRow, float DepthLayer)
{
	...
	m_pNew3DRenderMng->Add3DRenderObj(this, DepthLayer);

	LayerDepht = DepthLayer;
...
void SEASON3B::CNewUIInventoryCtrl::Render()
{
										...
										else if (pPickItem->Type == ITEM_POTION + 42)
										{
											if (pTargetItem->Jewel_Of_Harmony_Option == 0)
											{
												...
												if ((strengthitem != SI_None) &&
													((pTargetItem->ExtOption % 0x04) != EXT_A_SET_OPTION &&
														(pTargetItem->ExtOption % 0x04) != EXT_B_SET_OPTION))
												{
													bSuccess = true;
												}
											}
	...
	if (m_pNew3DRenderMng)
	{
		m_pNew3DRenderMng->RenderUI2DEffect(LayerDepht, UI2DEffectCallback, this, RENDER_NUMBER_OF_ITEM, 0);
