...
void CUISlideHelp::ManageSlide()
{
	...
	if (this == nullptr || m_pszSlideText == nullptr || m_pszSlideText == (char*)0x80808080 || m_pszSlideText == (char*)0xCDCDCD || m_pszSlideText == (char*)0xFEEEFEEE)
	{
		return; // objeto inválido, sai silenciosamente
	}
	...
	for (m_SlideQueueIter = m_SlideQueue.begin(); m_SlideQueueIter != m_SlideQueue.end(); )
	{
		if (m_SlideQueueIter->first > m_dwCurrentSecond)
			break;

		if (m_SlideQueueIter->second.m_iType != -1 || m_SlideQueueIter->first + 60 >= m_dwCurrentSecond)
		{
			if (AddSlideText(m_SlideQueueIter->second.m_pszText, m_SlideQueueIter->second.m_dwTextColor) == FALSE)
				break;
		...
		m_SlideQueueIter = m_SlideQueue.erase(m_SlideQueueIter);
	}
}
...
{
	int frame_cx = 2 * GetCenterX(640);

	EnableAlphaTest();
	SetLineColor(7, 0.4f);
	RenderColor(m_iPos_x - 1 + frame_cx, m_iPos_y - m_iHeight - 1, m_iWidth + 1, m_iHeight + 2);
	SetLineColor(0, 0.4f);
	RenderColor(m_iPos_x - 1 + frame_cx, m_iPos_y + 25 - 1, m_iWidth - 100 + 1, 20);

	SetLineColor(7, 0.4f);
	RenderColor(m_iPos_x - 1 + m_iWidth - 100 + 1 + frame_cx, m_iPos_y + 25 - 1, m_iWidth - 60, 20);
	EndRenderColor();
	...
	g_pGuardWindow->RenderScrollBarFrame(m_iPos_x + m_iWidth - 8 + frame_cx, m_fScrollBarRange_top, m_fScrollBarRange_bottom - m_fScrollBarRange_top);
	g_pGuardWindow->RenderScrollBar(m_iPos_x + m_iWidth - 12 + frame_cx, m_fScrollBarPos_y, (GetState() == UISTATE_SCROLL && MouseLButtonPush));
	g_pRenderText->RenderText(m_iPos_x + 5 + frame_cx, m_iPos_y - m_iHeight - 12, GlobalText[182]);
	g_pRenderText->RenderText(m_iPos_x + 80 + frame_cx, m_iPos_y - m_iHeight - 12, GlobalText[1603]);
	g_pRenderText->RenderText(m_iPos_x + 120 + frame_cx, m_iPos_y - m_iHeight - 12, GlobalText[1604]);
	g_pRenderText->RenderText(m_iPos_x + 18 + frame_cx, m_iPos_y + 31 - 1, GlobalText[1977]);
}
...
BOOL CUIBCGuildListBox::RenderDataLine(int iLineNumber)
{
	...
	{
		glColor4f(1.0f, 1.0f, 1.0f, 0.5f);
		RenderColor(m_iPos_x + frame_cx, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		g_pRenderText->SetTextColor(0, 0, 0, 255);
	}
	else
	...
	int iPos_x = m_iPos_x + 4 + frame_cx;
	int iPos_y = GetRenderLinePos_y(iLineNumber);
	...
	char Dummy[300];	if (Select_Guild == iLineNumber)
	{
		...
		g_pRenderText->RenderText(m_iPos_x + 60 + frame_cx, m_iPos_y + 31 - 1, Dummy );
	}
...
BOOL CUIBCGuildListBox::DoLineMouseAction(int iLineNumber)
{
	...
	if (::CheckMouseIn(m_iPos_x + frame_cx, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13))
	{
		if (MouseLButtonPush)
		{
