...
{
	// 채팅 타입 버튼
	RenderImage(IMAGE_INPUTBOX_NORMAL_ON + m_iInputMsgType, m_WndPos.x + BUTTON_WIDTH * m_iInputMsgType, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);

	// 귓속말 차단 On/Off
	if(m_bBlockWhisper)
	{
		RenderImage(IMAGE_INPUTBOX_WHISPER_ON, m_WndPos.x + BLOCK_WHISPER_START_X, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);
	}

	// 채팅 로그 출력 On/Off
	if(m_bShowChatLog)
	{
		RenderImage(IMAGE_INPUTBOX_CHATLOG_ON, m_WndPos.x + CHATLOG_ON_START_X, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);

		// 시스템 메세지만 출력 On/Off
		if(m_bOnlySystemMessage)
		{
			RenderImage(IMAGE_INPUTBOX_SYSTEM_ON, m_WndPos.x + SYSTEM_ON_START_X, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);
		}
	}
	...
	if(m_pNewUIChatLogWnd->IsShowFrame())
	{
		RenderImage(IMAGE_INPUTBOX_FRAME_ON, m_WndPos.x + FRAME_ON_START_X, m_WndPos.y, BUTTON_WIDTH, BUTTON_HEIGHT);
...
void SEASON3B::CNewUIChatInputBox::RenderTooltip()
{
	...
	Fontsize.cx = Fontsize.cx / ((float)WindowWidth / GetWindowsX);
	Fontsize.cy = Fontsize.cy / ((float)WindowHeight / 480);

	int x = m_WndPos.x + (m_iTooltipType * BUTTON_WIDTH) + (m_iTooltipType / 3 * GROUP_SEPARATING_WIDTH) + 10 - (Fontsize.cx / 2);
