...
bool SEASON3B::CNewUIMyQuestInfoWindow::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	//m_MoveWindow.Create(x, y, MYQUESTINFO_WINDOW_WIDTH, MYQUESTINFO_WINDOW_HEIGHT);
...
bool SEASON3B::CNewUIMyQuestInfoWindow::BtnProcess()
{
	...
	if (iIndex == TAB_JOB_CHANGE)
	{
		SendRequestEventCount(2);	// 블러드캐슬
		return true;
	}
	else if (iIndex == TAB_CASTLE_TEMPLE)
	{
...
#include <list> // Adicione esta linha para incluir a defini豫o de std::list
#include <string> // Adicione esta linha para incluir a defini豫o de std::string

void SEASON3B::CNewUIMyQuestInfoWindow::SetButtonInfo()
{
	#ifndef PJH_NEW_SERVER_SELECT_MAP
	std::list<unicode::t_string> TextLists;

	TextLists.push_back(GlobalText[1140]);

	TextLists.push_back(GlobalText[1142]);

	TextLists.push_back(GlobalText[2369]);

	m_BtnTab.CreateRadioGroup(3, IMAGE_MYQUEST_TAB_BUTTON);
	m_BtnTab.ChangeRadioText(TextLists);
	m_BtnTab.ChangeRadioButtonInfo(true, m_Pos.x + 12.f, m_Pos.y + 202.f, 56, 22);
