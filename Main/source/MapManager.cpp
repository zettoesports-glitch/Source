...
void CMapManager::Load() // OK
{
	...
	else
	{
		...
		CUSTOM_WORLD_INFO* customWorld = gCustomWorld.GetInfo(this->WorldActive);

		if (customWorld != NULL && customWorld->WorldFolder > 0)
		{
			iMapWorld = customWorld->WorldFolder;
		}
		else if (this->InBloodCastle() == true)
		{
	...
	if (g_MapProcess)
	{
		g_MapProcess->LoadMapData();
	}
}
static bool CheckWorldFileResult(int iResult, int iMapWorld, CUSTOM_WORLD_INFO* customWorld)
{
	// Mantém o comportamento original:
	// -1 significa que o arquivo não foi aberto/encontrado.
	// O código original não tratava -1 como "corrupted".
	if (iResult == -1)
	{
		return true;
	}

	// Mapas normais precisam bater exatamente.
	if (iResult == iMapWorld)
	{
		return true;
	}

	// Mapas custom podem reaproveitar arquivos de outro mapa.
	// Exemplo: World83 usando arquivo interno baseado em World91.
	// Então não podemos matar o cliente por diferença de ID interno.
	if (customWorld != NULL)
	{
		return true;
	}
...
void CMapManager::LoadWorld(int Map)
{
	...
	if (CheckWorldFileResult(iResult, iMapWorld, customWorld) == false)
	{
		char Text[256];
	...
	if (CreateWaterTerrain(this->WorldActive) == false)
	{
		...
		if (this->WorldActive == 0 || this->WorldActive == 3 || this->WorldActive == 63)
		{
			sprintf(FileName, "%s\\leaf01.tga", WorldName);
			LoadBitmap(FileName, BITMAP_LEAF1, GL_NEAREST, GL_CLAMP_TO_EDGE, false);
		}
...
const char* CMapManager::GetMapName(int iMap)
{
	...
	CUSTOM_WORLD_INFO* customWorld = gCustomWorld.GetInfo(iMap);
	if (customWorld != NULL && customWorld->TextIndex >= 0)
	{
		return GlobalText[customWorld->TextIndex];
	}
