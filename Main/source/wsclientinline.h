...
__forceinline void SendRequestChaosMix(int Flag)
{
	LogOut = true;
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF3);
	spe << (BYTE)0xF2 << (BYTE)Flag;
	spe.Send(TRUE);

	g_ConsoleDebug->Write(MCD_SEND, "0xF2 [SendRequestChaosMix]");
}
...
__forceinline void SendRequestEquipmentItem(int SrcFlag,int SrcIndex,int DstFlag,int DstIndex)
{
	...
	if (PickItem.Jewel_Of_Harmony_Option != 0 || PickItem.Jewel_Of_Harmony_OptionLevel != 0)
	{
		spareBits = (((BYTE)PickItem.Jewel_Of_Harmony_Option) << 4) + ((BYTE)PickItem.Jewel_Of_Harmony_OptionLevel);
	}
	else if (g_SocketItemMgr.IsSocketItem(&PickItem))
	{
...
{\
	...
	{\
		if(gProtect->m_MainInfo.m_NoDelayPotion != 0)\
		{\
			CStreamPacketEngine spe;\
			spe.Init( 0xC1, 0x26);\
