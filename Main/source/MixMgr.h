...
namespace SEASON3A
{
	...
	public:
		...
		int GetSuccessRate()
		{
			...
			if (gChaosMix->ChaosBoxMixCheck() != 0)
			{
				Rate = gChaosMix->MixRate;
			}
			else
			{
				Rate = m_MixRecipe[GetMixInventoryType()].GetSuccessRate();
			}
		...
		int GetReqiredZen()
		{
			...
			if (gChaosMix->ChaosBoxMixCheck() != 0)
			{
				Zen = gChaosMix->MixMoney;
			}
			else
			{
				Zen = m_MixRecipe[GetMixInventoryType()].GetReqiredZen();
			}
