...
bool CNewUIStorageInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if (false == m_pNewInventoryCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, x + 15, y + 36, 8, 15, this->GetLayerDepth() + 0.1f))
	{
	...
	for (int i = BTN_INSERT_ZEN; i < MAX_BTN; ++i)
	{
		...
		m_abtn[i].ChangeButtonInfo(x + anBtnPosX[i], y + 398, 36, 29);
