...
void CCharMakeWin::Create()
{
    ...
    {
        if (gProtect->m_MainInfo.m_DisableClassSUM != 0 && i == CLASS_SUMMONER) continue;
...
void CCharMakeWin::SetPosition(int nXCoord, int nYCoord)
{
    ...
    if (gProtect->m_MainInfo.m_DisableClassSUM == 0)
        m_abtnJob[CLASS_SUMMONER].SetPosition(nBaseX, nBaseY + 3 * nBtnHeight);
    else
        m_abtnJob[CLASS_DARK].SetPosition(nBaseX, nBaseY + 3 * nBtnHeight);
    ...
    {
        m_abtnJob[CLASS_DARK_LORD].SetPosition(nBaseX, nBaseY);
#ifdef PBG_ADD_NEWCHAR_MONK
        m_abtnJob[CLASS_RAGEFIGHTER].SetPosition(nBaseX, nBaseY + nBtnHeight);
#endif //PBG_ADD_NEWCHAR_MONK
    }
...
void CCharMakeWin::UpdateDisplay()
{
    ...
    if (gProtect->m_MainInfo.m_DisableClassSUM != 0)
    {
        m_abtnJob[CLASS_DARK].SetEnable(true);
        m_abtnJob[CLASS_DARK_LORD].SetEnable(true);
    }
    ...
    int nText;
    if (gProtect->m_MainInfo.m_DisableClassSUM == 0 && m_nSelJob == CLASS_SUMMONER)
        nText = 1690;
    else
        nText = 1705 + m_nSelJob;
    ...
    int i, j;
    {
        for (i = 0; i < MAX_CLASS; ++i)
        {
            ...
            if (m_abtnJob[i].IsClick())
            {
                ...
                {
                    if (gProtect->m_MainInfo.m_DisableClassSUM != 0 && j == CLASS_SUMMONER) continue;
...
void CCharMakeWin::RenderCreateCharacter()
{
    ...
    else if (gProtect->m_MainInfo.m_DisableClassSUM == 0 && CharacterView.Class == CLASS_SUMMONER)
    {
