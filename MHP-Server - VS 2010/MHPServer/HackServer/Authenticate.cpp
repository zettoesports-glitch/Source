...
bool CAuthenticate::Connect(char* IpAddress,WORD port) // OK
{
	...
	if(connect(this->m_socket,(SOCKADDR*)&target,sizeof(target)) == SOCKET_ERROR)
	{
		...
		{
			//gLog.Output(LOG_CONNECT,"[Connection] connect() falló con el error: %d",WSAGetLastError());
			//this->Disconnect();
			return 0;
		}
	}
...
bool CAuthenticate::DataRecv() // OK
{
	...
	if((result=recv(this->m_socket,(char*)&this->m_RecvBuff[this->m_RecvSize],(MAX_BUFF_SIZE-this->m_RecvSize),0)) == SOCKET_ERROR)
	{
		...
		{
			//gLog.Output(LOG_CONNECT,"[Connection] recv() falló con el error: %d",WSAGetLastError());
			this->Disconnect();
			return 0;
		}
	}
...
bool CAuthenticate::DataSend(BYTE* lpMsg,int size) // OK
{
	...
	while(size > 0)
	{
		if((result=send(this->m_socket,(char*)&buff[count],size,0)) == SOCKET_ERROR)
		{
			...
			{
				//gLog.Output(LOG_CONNECT,"[Connection] send() falló con el error: %d",WSAGetLastError());
				this->Disconnect();
				this->m_critical.unlock();
				return 0;
			}
...
DWORD WINAPI CAuthenticate::ClientWorkerThread(CAuthenticate* lpConnection) // OK
{
	while(true)
	{
		...
		if((NetworkEvents.lNetworkEvents & FD_READ) != 0)
		{
			...
			{
				//gLog.Output(LOG_CONNECT,"[Connection] FD_READ falló con el error: %d",NetworkEvents.iErrorCode[FD_READ_BIT]);
			}
		}
		...
		if((NetworkEvents.lNetworkEvents & FD_WRITE) != 0)
		{
			...
			{
				//gLog.Output(LOG_CONNECT,"[Connection] FD_WRITE falló con el error: %d",NetworkEvents.iErrorCode[FD_WRITE_BIT]);
			}
		}
		...
		if((NetworkEvents.lNetworkEvents & FD_CLOSE) != 0)
		{
			...
			{
				//gLog.Output(LOG_CONNECT,"[Connection] FD_CLOSE falló con el error: %d",NetworkEvents.iErrorCode[FD_CLOSE_BIT]);
			}
		}
	}
...
void CAuthenticate::AuthInfoRecv(PMSG_AUTH_INFO_RECV* lpMsg) // OK
{
	if(lpMsg->result == 0)
	{
		...
		{
			ErrorMessageBox("No puedes usar esta versión");
		}
	}
	else
	{
