...
void HackServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x11:
			...
			break;
		case 0x12:
			gProcessListInfo.RecvProcessList((PMSG_PROCESS_INFO_DATA_RECV*)lpMsg, index);
			break;
		case 0x13:
			gProcessListInfo.RecvProcessDumpRequest((PMSG_GET_PROCESS_DUMP_RECV*)lpMsg);
			break;
		case 0x15:
			gScreenShot.RecvInfo((PMSG_SCREEN_SHOT_SEND*)lpMsg, index);
			break;
		case 0x16:
			CGAccountInfoRecv((SDHP_CLIENT_ACCOUNT_INFO_RECV*)lpMsg, index);
			break;
		case 0x17:
			gProcessListInfo.RecvProcessClass((PMSG_WINDOW_CLASS_SEND*)lpMsg);
			break;
		case 0x18:
			gProcessListInfo.RecvFile((PMSG_FILE_RECV*)lpMsg);
			break;
		default:
			LogAdd(LOG_RED,"Paquete Incorrecto");
...
void CHClientInfoRecv(SDHP_CLIENT_INFO_RECV* lpMsg,int index) // OK
{
	...
	//gScreenShow.display_count = lpMsg->DisplayCount;
