...
void CReadFiles::UpdateInternalList()
{
	...
	{
		LogAdd(LOG_BLACK,"[Base de datos] Espere la actualización de la lista actual");
		return;
	}

	if(this->ReloadSwitch != 0)
	{
		LogAdd(LOG_BLACK,"[Base de datos] Espere a que se vuelva a cargar la lista actual");
		return;
	}
	...
	else
	{
		...
		LogAdd(LOG_RED,"[Database] No se pudo actualizar la lista (Error: %i)", httpLastError());
	...
	LogAdd(LOG_GREEN,"[Base de datos] Actualizada exitosamente");
