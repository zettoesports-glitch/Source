					...
					if(GetModuleBaseName(ProcessHandle,0,ProcessName,MAX_PATH) != 0)
					{
						if(strcmp(ProcessName,"GameServer.exe") == 0)
						{
							if(this->CheckProcessModule(ProcessHandle,ModulePath) == 0)
							{
								...
								{
									LogAdd(LOG_RED,"[ProcessManager][%d][%s] ¡No se pudo inicializar el proceso!",ProcessIds[n],ProcessName);
									CloseHandle(ProcessHandle);
								}
								else
								{
									LogAdd(LOG_BLUE,"[ProcessManager][%d][%s] ¡Inicialización exitosa!",ProcessIds[n],ProcessName);
									CloseHandle(ProcessHandle);
								}
							}
							else
