...
void SplashScreen(CSplash* ClassInst, int SplashType, bool TextSwitch, char DisplayText[255], int CloseTime)
{
    ...
    // Caso especial: BLOQUEADO (SplashType == 1 - o que tem a cor amarela)
    // Não carrega o splash, exibe MessageBox diretamente e encerra
    if (SplashType == 1)
    {
        MessageBox(
            NULL,
            TEXT("Este computador está bloqueado"),
            TEXT("Acesso Negado"),
            MB_OK | MB_ICONSTOP | MB_TOPMOST | MB_SETFOREGROUND
        );

        // Após clicar em OK, fecha e encerra o processo
    ...
    // === CASOS NORMAIS: SplashType 0 e 2 (comportamento original) ===
    int ResId;
    COLORREF TextColor;
    ...
    if(CloseTime > 0)
    {
        ...
        if(SplashType != 0)  // Mantém o comportamento original para tipo 2
        {
            ClassInst->CloseSplash();
            SafeExitProcess();
        }
