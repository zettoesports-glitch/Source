...
bool CDumpCheck::Check(wchar_t* sTemp,char* CaptionName)
{
	...
	//gLog.Output(LOG_DEBUG,"isX64 = %d sTemp: %ls",gProtect.is_x64,sTemp);
	//MessageBox(0,"kek","da",0);
	...
	for(std::vector<DUMP_LIST_INFO>::iterator it=gListManager.gDumpListInfo.begin();it != gListManager.gDumpListInfo.end();it++)
	{
		...
		{
			//auto library = LoadLibrary("MHPx64.dll");
			if(load == 0)
			{
				typedef bool (__cdecl *pGetProcInfo)(wchar_t*);
				FARPROC pGetProcInfoAddress = GetProcAddress(gProtect.x64_library, "GetProcInfo");
				
				
				//if(this->GetProcessInfo(sTemp,&file,&mapping,&lpBaseAddress,&lpNtHeader) == 0)
				if( ((pGetProcInfo)pGetProcInfoAddress())(sTemp) == 0 )
				{
					break;
				}
				else
			...
			typedef bool (__cdecl *pFindEntryPointInfo)(wchar_t*,DWORD,BYTE*);
			FARPROC pFindEntryPointInfoAddress = GetProcAddress(gProtect.x64_library, "FindEntryPointInfo");
			
			//if(this->FindEntryPointInfo(sTemp,file,mapping,lpBaseAddress,lpNtHeader,it->offset,dump) == 0)
			if( ((pFindEntryPointInfo)pFindEntryPointInfoAddress())(sTemp,it->offset,dump) == 0 )
			{
				continue;
			}
