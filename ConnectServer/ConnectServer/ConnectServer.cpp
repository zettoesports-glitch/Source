...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	GetPrivateProfileString("ConnectServerInfo","CustomerName","",CustomerName,sizeof(CustomerName),".\\Data\\ConnectServer.ini");

	GetPrivateProfileString("ConnectServerInfo","CustomerHardwareId","",CustomerHardwareId,sizeof(CustomerHardwareId),".\\Data\\ConnectServer.ini");
	...
	wsprintf(buff,"[%s] MuOnline ConnectServer (QueueSize : %d)",CONNECTSERVER_VERSION,0);
	...
	{
		WORD ConnectServerPortTCP = GetPrivateProfileInt("ConnectServerInfo","ConnectServerPortTCP",44405,".\\Data\\ConnectServer.ini");

		WORD ConnectServerPortUDP = GetPrivateProfileInt("ConnectServerInfo","ConnectServerPortUDP",55557,".\\Data\\ConnectServer.ini");

		MaxIpConnection = GetPrivateProfileInt("ConnectServerInfo","MaxIpConnection",0,".\\Data\\ConnectServer.ini");

		if(gSocketManager.Start(ConnectServerPortTCP) != 0)
		{
			if(gSocketManagerUdp.Start(ConnectServerPortUDP) != 0)
			{
				gServerList.Load("Data\\ServerList.dat");
