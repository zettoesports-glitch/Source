// Generated from terrains/terrainheaven.vs. Edit the HLSL source and regenerate this file.
...
layout(location = 4) out vec4 output_LightPos;
layout(location = 5) out vec4 output_Color;
...
PixelInput _main(VertexInput _input)
{
    ...
    _output.LightPos = worldPos * spvWorkaroundRowMajor(GlobalConstantsData.Shadow);
    ...
    {
        float GrassWind = (sin(GlobalConstantsData.WindSpeed + (float(terrainPosition.x) * 50.0)) * GlobalConstantsData.WindScale) * 0.00200000009499490261077880859375;
        ...
        {
            _output.UV2 = vec2((float(terrainPosition.x) * _input.UV.y) + GlobalConstantsData.WaterMove, (float(terrainPosition.y) * _input.UV.y) + GrassWind);
        }
        else
        {
...
void main()
{
    ...
    output_LightPos = flattenTemp.LightPos;
