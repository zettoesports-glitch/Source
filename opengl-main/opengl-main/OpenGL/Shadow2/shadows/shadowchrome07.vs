// Generated from shadows/shadowchrome07.vs. Edit the HLSL source and regenerate this file.
...
float GetTerrainHeight(inout vec2 Pos)
{
    Pos /= vec2(100.0);
    ivec2 iPos = ivec2(Pos);
    iPos.x &= 255;
    iPos.y &= 255;
    vec2 diff = Pos - vec2(iPos);
    float h1 = texelFetch(HeightTexture, ivec3(uvec3(uvec2(iPos), 0u)).xy, ivec3(uvec3(uvec2(iPos), 0u)).z).x;
    float h2 = texelFetch(HeightTexture, ivec3(uvec3(uvec2(iPos + ivec2(0, 1)), 0u)).xy, ivec3(uvec3(uvec2(iPos + ivec2(0, 1)), 0u)).z).x;
    float h3 = texelFetch(HeightTexture, ivec3(uvec3(uvec2(iPos + ivec2(1, 0)), 0u)).xy, ivec3(uvec3(uvec2(iPos + ivec2(1, 0)), 0u)).z).x;
    float h4 = texelFetch(HeightTexture, ivec3(uvec3(uvec2(iPos + ivec2(1)), 0u)).xy, ivec3(uvec3(uvec2(iPos + ivec2(1)), 0u)).z).x;
...
ShadowPixelInput _main(VertexInput _input)
{
    ...
    else
    {
        ...
        mat4 _400 = RequestBone(param, param_1);
        mat4 BoneMatrix = _400;
        ...
        if (_input.Bones.x != _input.Bones.y)
        {
            ...
            mat4 _423 = RequestBone(param_2, param_3);
            BoneMatrix = _423;
        }
        ...
        vec4 _453 = _output.Position;
        vec3 _455 = _453.xyz * _input.BodyScale.x;
        _output.Position.x = _455.x;
        _output.Position.y = _455.y;
        _output.Position.z = _455.z;
        vec4 _465 = _output.Position;
        vec3 _467 = _465.xyz + _input.BodyOrigin;
        _output.Position.x = _467.x;
        _output.Position.y = _467.y;
        _output.Position.z = _467.z;
        ...
        _output.Position += bodyOrigin;
        vec2 param_4 = _output.Position.xy;
        float _510 = GetTerrainHeight(param_4);
        _output.Position.z = _510 + 6.0;
        ...
        _output.UV = ((normal.zz + normal.xx) * 0.800000011920928955078125) + (vec2(GlobalConstantsData.WorldTime, GlobalConstantsData.WorldTime) * 5.9999998484272509813308715820312e-05);
