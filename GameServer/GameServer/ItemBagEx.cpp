		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if (section == 4)
				{
					...
					info.VIP = lpMemScript->GetAsNumber();
					info.VipExtraRate = lpMemScript->GetAsNumber();
				...
				{
					char ItemIndexText[32];
					strcpy_s(ItemIndexText, lpMemScript->GetAsString());
					if (strcmp("end", ItemIndexText) == 0)
					{
						break;
					}
					...
					int ItemType = 0;
					int ItemID = 0;
					if (sscanf_s(ItemIndexText, "%d,%d", &ItemType, &ItemID) == 2)
					{
						info.Index = (ItemType * 512) + ItemID;
					}
					else
					{
						info.Index = atoi(ItemIndexText);
					}
					...
					info.ShowMessage = lpMemScript->GetAsNumber();
...
bool CItemBagEx::GetItem(LPOBJ lpObj, CItem* lpItem) // OK
{
	for (std::map<int, ITEM_BAG_EX_INFO>::iterator it = this->m_ItemBagInfo.begin(); it != this->m_ItemBagInfo.end(); it++)
	{
		...
		if ((GetLargeRand() % 10000) < DropRate)
		{
			...
			for (std::vector<ITEM_BAG_EX_DROP_INFO>::iterator DropInfo = it->second.DropInfo.begin(); DropInfo != it->second.DropInfo.end(); DropInfo++)
			{
				...
				int FinalSectionRate = DropInfo->SectionRate;

				if (lpObj->AccountLevel >= DropInfo->VIP && DropInfo->VipExtraRate > 0)
				{
					FinalSectionRate += DropInfo->VipExtraRate;
				}

				RandomManager.AddElement((int)(&(*DropInfo)), FinalSectionRate);
			}
			...
			lpItem->Convert(ItemIndex, ItemOption1, ItemOption2, ItemOption3,
				((ItemNewOption == 0) ? lpItemBagItemInfo->Grade : ItemNewOption),
...
bool CItemBagEx::DropItem(LPOBJ lpObj, int map, int x, int y) // OK
{
	...
	for (std::map<int, ITEM_BAG_EX_INFO>::iterator it = this->m_ItemBagInfo.begin(); it != this->m_ItemBagInfo.end(); it++)
	{
		...
		if ((GetLargeRand() % 10000) < DropRate)
		{
			...
			GDCreateItemSend(lpObj->Index, map, px, py, ItemIndex, ItemLevel, 0, ItemOption1, ItemOption2, ItemOption3,
				lpObj->Index, ((ItemNewOption == 0) ? lpItemBagItemInfo->Grade : ItemNewOption),
				ItemSetOption, 0, 0, ItemSocketOption, 0xFF,
			...
			if (lpItemBagItemInfo->ShowMessage == 1)
			{
				...
				{
					int FinalSectionRate = lpItemBagDropInfo->SectionRate;
					if (lpObj->AccountLevel >= lpItemBagDropInfo->VIP && lpItemBagDropInfo->VipExtraRate > 0)
					{
						FinalSectionRate += lpItemBagDropInfo->VipExtraRate;
					}

					int percent = (int)(FinalSectionRate / 100.0f);

					char szPercent[32];
					sprintf_s(szPercent, "%d", percent);

					char buff[256];
					wsprintf(buff, gMessage.GetMessage(665), lpObj->Name, ItemInfo.Name, szPercent);
