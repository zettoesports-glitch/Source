...
bool CCustomPick::CommandPick(LPOBJ lpObj, char* arg)
{
	...
	// Verifica se o mapa permite o uso do Custom Pick
	...
	// Verifica se o jogador está em uma condição que impede o uso do comando
	if (lpObj->Interface.use != 0 || lpObj->Teleport != 0 || lpObj->DieRegen != 0 || lpObj->PShopOpen != 0)
	{
	...
	// Modo para pegar todos os itens da lista CUSTOMPICK_INFO
	if (strcmp(mode, "all") == 0)
	{
		// Limpa todos os itens atuais do Pickup
		for (int i = 0; i < MAX_CUSTOMPICK; i++)
		{
			lpObj->Pickup[i] = -1;
		}

		// Itera sobre todos os itens carregados no CUSTOMPICK_INFO
		int pickupIndex = 0;
		for (std::map<int, CUSTOMPICK_INFO>::iterator it = this->m_CustomPickInfo.begin(); it != this->m_CustomPickInfo.end(); it++)
		{
			if (pickupIndex >= MAX_CUSTOMPICK)
			{
				break; // Limite máximo de itens atingido
			}

			// Adiciona o item ao Pickup
			lpObj->Pickup[pickupIndex] = (it->second.Cat * 512) + it->second.Item;
			pickupIndex++;
		}

		// Verifica se o Pickup já está ativado
		if (lpObj->PickupEnable == 1) {
			// Desativa o Pickup
			lpObj->PickupEnable = 0;
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "PickUp [ All ] desativado!");

			gLog.Output(LOG_COMMAND, "[CustomPick][%s][%s] - (Disable: [ All ])", lpObj->Account, lpObj->Name);
			return 1;
		}
		else {
			// Ativa o Pickup
			lpObj->PickupEnable = 1;
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "PickUp [ All ] ativado!");

			gLog.Output(LOG_COMMAND, "[CustomPick][%s][%s] - (Enable: [ All ])", lpObj->Account, lpObj->Name);
			return 1;
		}
	}

	// Modos existentes (exc, socket, setitem)
	...
	// Modo para itens personalizados (já existente)
	if (strcmp(mode, "") != 0)
	{
