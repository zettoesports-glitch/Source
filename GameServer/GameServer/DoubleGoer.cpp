...
void CDoubleGoer::ProcState_CLEAN() // OK
{
	...
	if (remainMinutes >= 1 && remainMinutes <= 3)
	{
		...
		{
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(663), remainMinutes);
...
void CDoubleGoer::CGDoubleGoerEnterRecv(PMSG_DOUBLE_GOER_ENTER_RECV* lpMsg,int aIndex) // OK
{
    ...
    if(gMapServerManager.CheckMapServer(MAP_DOUBLE_GOER1) == 0 ||
       gMapServerManager.CheckMapServer(MAP_DOUBLE_GOER2) == 0 ||
       gMapServerManager.CheckMapServer(MAP_DOUBLE_GOER3) == 0 ||
       gMapServerManager.CheckMapServer(MAP_DOUBLE_GOER4) == 0)
    {
        pMsg.result = 2;
        DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
        return;
    ...
    if(OBJECT_RANGE(this->m_PartyNumber) != 0)
    {
        ...
        {
            int memberIndex = gParty.m_PartyInfo[this->m_PartyNumber].Index[n];
            if(OBJECT_RANGE(memberIndex) == 0 || memberIndex == aIndex)
            {
                continue;
            }

            bool hasTicket = false;
            for(int slot=INVENTORY_WEAR_SIZE; slot < INVENTORY_FULL_SIZE; slot++)
            {
                if(gObj[memberIndex].Inventory[slot].IsItem() == 0)
                {
                    continue;
                }
                if(gObj[memberIndex].Inventory[slot].m_Index == GET_ITEM(14,111) || 
                   gObj[memberIndex].Inventory[slot].m_Index == GET_ITEM(13,125))
                {
                    hasTicket = true;
                    gItemManager.DecreaseItemDur(&gObj[memberIndex],slot,1);
                    break;
                }
            }
            ...
            if(hasTicket)
            {
                if(this->AddUser(memberIndex) != 0)
                {
                    gObj[memberIndex].Interface.use = 0;
                    gObj[memberIndex].Interface.type = INTERFACE_NONE;
                    gObj[memberIndex].Interface.state = 0;
                    gObjMoveGate(memberIndex,329+this->m_Level);
                    this->GCDoubleGoerStateSend(0,memberIndex);
                    this->m_PartyCount++;
                    if((gObj[memberIndex].Level + gObj[memberIndex].MasterLevel) > this->m_HighestLevel)
                    {
                        this->m_HighestLevel = gObj[memberIndex].Level + gObj[memberIndex].MasterLevel;
                    }
                }
            }
        }
