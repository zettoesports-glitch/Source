...
void CMapManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.CustomOffStore = lpMemScript->GetAsNumber();

			info.CustomAttack = lpMemScript->GetAsNumber();

			info.CustomOffAttack = lpMemScript->GetAsNumber();

			info.CustomPick = lpMemScript->GetAsNumber();

			info.MoneyDrop = lpMemScript->GetAsNumber();
...
int CMapManager::GetMapCustomOffStore(int index) // OK
{
	std::map<int, MAP_MANAGER_INFO>::iterator it = this->m_MapManagerInfo.find(index);
	...
	{
		return ((it->second.CustomOffStore == -1) ? 0 : it->second.CustomOffStore);
	}
}
...
int CMapManager::GetMapCustomOffAttack(int index) // OK
{
	std::map<int, MAP_MANAGER_INFO>::iterator it = this->m_MapManagerInfo.find(index);
	...
	{
		return ((it->second.CustomOffAttack == -1) ? 0 : it->second.CustomOffAttack);
	}
}
...
int CMapManager::GetMapMoneyDrop(int index) // OK
{
	std::map<int, MAP_MANAGER_INFO>::iterator it = this->m_MapManagerInfo.find(index);
	...
	{
		return ((it->second.MoneyDrop == -1) ? 1 : it->second.MoneyDrop);
	}
}
