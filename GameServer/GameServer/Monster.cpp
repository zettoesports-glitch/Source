...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	ITEM_BAG_MANAGER_INFO bagInfo;
	bool hasBag = false;

	for (auto& it : gItemBagManager.m_ItemBagManagerInfo)
	{
	...
	if (hasBag)
	{
		int modo = bagInfo.TopHitDrop;

		int maxRecipients = (modo > 3) ? 3 : modo;
		...
		if (modo >= 1 && modo <= 999)
		{
			...
			{
				int idx = lpObj->HitDamage[n].index;
				if (OBJECT_RANGE(idx) &&
					gObjIsConnected(idx) &&
			...
			if (cont > 0)
			{
				...
				int qtd = min(maxRecipients, cont);

				for (int pos = 0; pos < qtd; pos++)
				{
					int topIdx = entries[pos].index;
					if (OBJECT_RANGE(topIdx) && gObjIsConnected(topIdx))
					{
						...
						if (dropY < 0) dropY = 0;
						if (dropY >= 255) dropY = 254;

						bagInfo.ItemBag.DropItem(lpTop, lpObj->Map, dropX, dropY);
					}
				}
	...
	else if (lpObj->Money > 0 && (GetLargeRand() % ((lpObj->MoneyRate == 0) ? 1 : lpObj->MoneyRate)) < 10)
	{
		...
		//lpObj->GiveItemDrop = true; //-- add here
	}
}
	...
	if (lpObj->Type == OBJECT_MONSTER && lpObj->HitDamageCount > 0)
	{
		...
		if (gCustomMonster.GetCustomMonsterInfo(lpObj->Class, lpObj->Map, &CustomMonsterInfo) != 0)
		{
			if (CustomMonsterInfo.ShowTopDamage == 1)
			{
				...
				for (int n = 0; n < MAX_HIT_DAMAGE; n++)
				{
					int aIndex = lpObj->HitDamage[n].index;
				...
				int placesToShow = min(validCount, 3);
				char szMsg[160];
				char formattedDmg[32];
				...
				for (int pos = 0; pos < placesToShow; pos++)
				{
					...
					LPOBJ lpPlayer = &gObj[aIndex];

					FormatDamageNumber(rawDmg, formattedDmg, sizeof(formattedDmg));

					int msgId = 671 + pos;

					wsprintf(szMsg, gMessage.GetMessage(msgId), lpPlayer->Name, formattedDmg);

					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, szMsg);
				}
			}
		}
	}
