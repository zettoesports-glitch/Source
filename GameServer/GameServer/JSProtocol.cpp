...
void JoinServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x05:
			...
			break;
		case 0x06:
			JGAccountLevelVipRecv((SDHP_ACCOUNT_LEVEL_RECV*)lpMsg);
			break;
		case 0x2F:
...
void JGAccountLevelVipRecv(SDHP_ACCOUNT_LEVEL_RECV* lpMsg) // OK
{
	if (strcmp(lpMsg->account, "null") == 0)
	{
		LogAdd(LOG_RED, "[JGAccountLevelVipRecv] Invalid Account [%d](%s)", lpMsg->index, lpMsg->account);
		CloseClient(lpMsg->index);
		return;
	}
	else
