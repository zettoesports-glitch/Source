...
void CMonsterSetBase::LoadFolder(const char* folder)
{
	this->m_count = 0;

	this->LoadFolderRecursive(folder);
}

void CMonsterSetBase::LoadFolderRecursive(const char* folder)
{
	...
	do
	{
		...
		std::string fullPath = std::string(folder) + "\\" + ffd.cFileName;

		if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
		{
			LoadFolderRecursive(fullPath.c_str());
		}
		else
		{
			const char* ext = strrchr(ffd.cFileName, '.');
			if (ext != nullptr && _stricmp(ext, ".txt") == 0)
			{
				this->LoadFile(fullPath.c_str(), false);
			}
		}
...
void CMonsterSetBase::LoadFile(const char* path, bool clear)
{
	CMemScript* lpMemScript = new CMemScript;

	if (lpMemScript == 0)
	{
		ErrorMessageBox(MEM_SCRIPT_ALLOC_ERROR, (char*)path);
		return;
	}
...
bool CMonsterSetBase::GetBoxPosition(int map, int x, int y, int tx, int ty, short* ox, short* oy) // OK
{
	for (int n = 0; n < 100; n++)
	{
		...
		if (gMap[map].CheckAttr(subx, suby, 1) == 0 &&
			gMap[map].CheckAttr(subx, suby, 4) == 0 &&
			gMap[map].CheckAttr(subx, suby, 8) == 0)
		{
			(*ox) = subx;
			(*oy) = suby;
			return 1;
