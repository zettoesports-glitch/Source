...
BYTE CItemManager::MoveItemToInventoryFromChaosBoxGenesis(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag)
{
	...
	if (this->InventoryAddItem(lpObj->Index, lpObj->ChaosBoxGenesis[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
...
void CItemManager::ItemByteConvert(BYTE* lpMsg,CItem item) // OK
{
	...
	if (gServerInfo.m_AllowSocketHarmony == 1)
	{
...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gLuaGameServer.PlayerPickItem(aIndex, index) == 1)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg, int aIndex) // OK
{
	...
	{
		if (gMap[lpObj->Map].ItemDrop(lpItem->m_Index, lpItem->m_Level, lpItem->m_Durability, lpMsg->x, lpMsg->y,
			lpItem->m_Option1, lpItem->m_Option2, lpItem->m_Option3, lpItem->m_NewOption, lpItem->m_SetOption,
			lpItem->m_Serial, aIndex, lpItem->m_PetItemLevel, lpItem->m_PetItemExp, lpItem->m_JewelOfHarmonyOption,
			lpItem->m_ItemOptionEx, lpItem->m_SocketOption, lpItem->m_SocketOptionBonus, lpItem->m_PeriodicItemTime) == 0)
		{
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg, int aIndex) // OK
{
	...
	if (pMsg.result != 0xFF)
	{
		...
		/*
		else if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 38)
		{
			gLuaGameServer.PlayerMoveItem(aIndex, lpMsg->SourceSlot, lpMsg->TargetSlot, 7); //TradeX > Inventory
			gDeathSystem.SendInformation(lpObj, 6);
			gDeathSystem.SendInformation(lpObj, 0);
			gDeathSystem.SendTradeTargetX(lpObj);
		}
		else if (lpMsg->SourceFlag == 38 && lpMsg->TargetFlag == 0)
		{
