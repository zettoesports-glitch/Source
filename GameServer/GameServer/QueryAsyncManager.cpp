// QueryAsyncManager.cpp: implementation of the CQueryAsyncManager class.
//
//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
bool CQueryAsyncManager::Connect(char* odbc, char* user, char* pass) // OK
{
...
void CQueryAsyncManager::Disconnect() // OK
{
...
void CQueryAsyncManager::Diagnostic() // OK
{
	...
	{
		LogAdd(LOG_RED, "[QueryAsyncManager] State (%s), Diagnostic: %s", SqlState, MessageText);
	}
...
bool CQueryAsyncManager::ExecQuery(std::string query) // OK
{
	SQLRETURN result;
...
void CQueryAsyncManager::Close() // OK
{
	SQLCloseCursor(this->m_STMT);
	SQLFreeStmt(this->m_STMT, SQL_UNBIND);
}

SQLRETURN CQueryAsyncManager::Fetch() // OK
{
	return SQLFetch(this->m_STMT);
}

int CQueryAsyncManager::FindIndex(char* ColName) // OK
{
	for (int n = 0; n < this->m_ColCount; n++)
	{
...
int CQueryAsyncManager::GetResult(int index) // OK
{
	return atoi(this->m_SQLData[index]);
}

int CQueryAsyncManager::GetAsInteger(char* ColName) // OK
{
...
float CQueryAsyncManager::GetAsFloat(char* ColName) // OK
{
	int index = this->FindIndex(ColName);
...
__int64 CQueryAsyncManager::GetAsInteger64(char* ColName) // OK
{
	int index = this->FindIndex(ColName);
...
void CQueryAsyncManager::GetAsString(char* ColName, char* OutBuffer, int OutBufferSize) // OK
{
	int index = this->FindIndex(ColName);
...
void CQueryAsyncManager::GetAsBinary(char* ColName, BYTE* OutBuffer, int OutBufferSize) // OK
{
	int index = this->FindIndex(ColName);
...
void CQueryAsyncManager::BindParameterAsString(int ParamNumber, void* InBuffer, int ColumnSize) // OK
{
...
void CQueryAsyncManager::BindParameterAsBinary(int ParamNumber, void* InBuffer, int ColumnSize) // OK
{
...
void CQueryAsyncManager::ConvertStringToBinary(char* InBuff, int InSize, BYTE* OutBuff, int OutSize) // OK
{
	int size = 0;
...
void CQueryAsyncManager::ConvertBinaryToString(BYTE* InBuff, int InSize, char* OutBuff, int OutSize) // OK
{
	int size = 0;
