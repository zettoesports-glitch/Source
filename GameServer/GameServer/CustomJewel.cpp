		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					info.EnableSlotPets = lpMemScript->GetAsNumber();
...
bool CCustomJewel::CheckCustomJewelApplyItem(int ItemIndex, CItem* lpItem) // OK
{
	...
	bool IsWingItem = (
		((lpItem->m_Index >= GET_ITEM(12, 0) && lpItem->m_Index <= GET_ITEM(12, 6)) ||
			(lpItem->m_Index >= GET_ITEM(12, 36) && lpItem->m_Index <= GET_ITEM(12, 43)) ||
			lpItem->m_Index == GET_ITEM(12, 49) ||
			lpItem->m_Index == GET_ITEM(12, 50) ||
			(lpItem->m_Index >= GET_ITEM(12, 262) && lpItem->m_Index <= GET_ITEM(12, 265)) ||
			lpItem->m_Index == GET_ITEM(13, 30))
		);

	bool IsRingItem = (
		((lpItem->m_Index >= GET_ITEM(13, 20) && lpItem->m_Index <= GET_ITEM(13, 28)) ||
	...
	bool IsPetItem = (
		((lpItem->m_Index >= GET_ITEM(13, 0) && lpItem->m_Index <= GET_ITEM(13, 5)) ||
			lpItem->m_Index == GET_ITEM(13, 37) ||
			lpItem->m_Index == GET_ITEM(13, 64) ||
			lpItem->m_Index == GET_ITEM(13, 65) ||
			lpItem->m_Index == GET_ITEM(13, 67) ||
			(lpItem->m_Index >= GET_ITEM(13, 71) && lpItem->m_Index <= GET_ITEM(13, 75)))
		);

	bool IsCustomWingItem = (gCustomWing.CheckCustomWingByItem(lpItem->m_Index) != 0);

	if (lpItem->m_Index > GET_ITEM(12, 6) && IsWingItem == 0 && IsRingItem == 0 && IsPetItem == 0 && IsCustomWingItem == 0)
	{
		return 0;
	}
	...
	if (lpInfo->EnableSlotWing == 0 && (IsWingItem != 0 || IsCustomWingItem != 0))
	{
		return 0;
	}

	if (lpInfo->EnableSlotRing == 0 && IsRingItem != 0)
	{
		return 0;
	}

	if (lpInfo->EnableSlotPets == 0 && IsPetItem != 0)
	{
		return 0;
	}
...
int CCustomJewel::GetCustomJewelNewOption(CItem* lpItem,int value) // OK
{
	...
	while (true) //Fix Excelent em Asas
	{
		int NewOptionCount = GetNewOptionCount(
			((lpItem->m_Index >= GET_ITEM(12, 0) && lpItem->m_Index <= GET_ITEM(12, 6)) ||
				(lpItem->m_Index >= GET_ITEM(12, 36) && lpItem->m_Index <= GET_ITEM(12, 43)) ||
				lpItem->m_Index == GET_ITEM(12, 49) ||
				lpItem->m_Index == GET_ITEM(12, 50) ||
			...
			? (lpItem->m_NewOption & 0x0F)
			: lpItem->m_NewOption
		);

		if (count >= value || NewOptionCount >= MaxNewOption)
		{
			break;
		}
