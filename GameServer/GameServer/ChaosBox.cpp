...
void CChaosBox::CGChaosMixRateRecv(PMSG_CHAOS_MIX_RATE_RECV* lpMsg, int aIndex) // OK
{
	...
	if (gCustomMix.CheckCustomMix(lpMsg->type) != 0)
	{
		pMsg.rate = gCustomMix.GetCustomMixSuccessRate(lpMsg->type, lpObj->AccountLevel);

		pMsg.rate = ((pMsg.rate > 100) ? 100 : pMsg.rate);

		pMsg.money = gCustomMix.GetCustomMixMoney(lpMsg->type);
...
void CChaosBox::CustomMixRate(LPOBJ lpObj, int type, int rate, int money, int tax) // OK
{
	rate = gCustomMix.GetCustomMixSuccessRate(type, lpObj->AccountLevel);

	rate = ((rate > 100) ? 100 : rate);

	money = gCustomMix.GetCustomMixMoney(type);

	money = rate * 10000;

	tax = (money * gCastleSiegeSync.GetTaxRateChaos(lpObj->Index)) / 100;
}