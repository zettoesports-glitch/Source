...
void GCLifeSend(int aIndex, BYTE type, int life, int shield) // OK
{
	...
	pMsg.life = GET_MAX_WORD_VALUE(life);

	pMsg.shield = GET_MAX_WORD_VALUE(shield);
...
void GCManaSend(int aIndex, BYTE type, int mana, int bp) // OK
{
	...
	pMsg.mana = GET_MAX_WORD_VALUE(mana);

	pMsg.bp = GET_MAX_WORD_VALUE(bp);
...
void GCNewHealthBarSend(LPOBJ lpObj) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		if (lpObj->VpPlayer[n].type == OBJECT_USER && gServerInfo.m_PlayerHealthBarSwitch == 0)
		{
			continue;
		}
