...
#if defined(LUA_VERSION_NUM) && LUA_VERSION_NUM > 500 && !defined(LUA_COMPAT_MODULE)
...
namespace LUAAA_NS
{
	...
	{
		inline static long long get(lua_State * L, long long idx)
		{
			if (lua_isnumber(L, (int)idx) || lua_isstring(L, (int)idx))
			{
				return long long(lua_tointeger(L, (int)idx));
			}
			else
			{
				luaL_checktype(L, (int)idx, LUA_TNUMBER);
			}
			return 0;
		}

		inline static void put(lua_State * L, const long long & t)
		{
			lua_pushinteger(L, t);
		}
	};
	...
	{
		inline static short get(lua_State * L, short idx)
		{
			if (lua_isnumber(L, idx) || lua_isstring(L, idx))
			{
				return short(lua_tointeger(L, idx));
			}
			else
			{
				luaL_checktype(L, idx, LUA_TNUMBER);
	...
	{
		inline static unsigned short get(lua_State * L, unsigned short idx)
		{
			if (lua_isnumber(L, idx) || lua_isstring(L, idx))
			{
				return unsigned short(lua_tointeger(L, idx));
			}
			else
			{
				luaL_checktype(L, idx, LUA_TNUMBER);
