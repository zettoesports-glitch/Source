...
BOOL ObjBotBuffer::TradeOpen(int index, int nindex)
{
	...
	{
		//gNotice.NewMessageDevTeam(lpObj->Index, gNewMessage.GetTextMessage(521, lpObj->Language), this->bot[number].Name);
		return 1;
	}
	if (this->bot[number].OnlyVip != 0 && lpObj->AccountLevel == 0)
	{
		//gNotice.NewMessageDevTeam(gObj[nindex].Index, gNewMessage.GetTextMessage(522, gObj[nindex].Language), this->bot[number].Name);
		return 1;
	}
	if ((DWORD)this->bot[number].Zen > gObj[index].Money)
	{
		//gNotice.NewMessageDevTeam(gObj[index].Index, gNewMessage.GetTextMessage(523, gObj[index].Language), this->bot[number].Name, this->bot[number].Zen);
		return 1;
	}
	if (this->bot[number].ActiveGensFamily == 1)
	{
		...
		{
			//gNotice.NewMessageDevTeam(gObj[index].Index, gNewMessage.GetTextMessage(525, gObj[index].Language), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily != GENS_FAMILY_DUPRIAN && gObj[index].GensFamily == GENS_FAMILY_NONE)
		{
			//gNotice.NewMessageDevTeam(gObj[index].Index, gNewMessage.GetTextMessage(526, gObj[index].Language), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily == GENS_FAMILY_VARNERT && gObj[index].GensFamily == GENS_FAMILY_DUPRIAN)
		{
			//gNotice.NewMessageDevTeam(gObj[index].Index, gNewMessage.GetTextMessage(527, gObj[index].Language), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].GensFamily == GENS_FAMILY_DUPRIAN && gObj[index].GensFamily == GENS_FAMILY_VARNERT)
		{
			//gNotice.NewMessageDevTeam(gObj[index].Index, gNewMessage.GetTextMessage(528, gObj[index].Language), this->bot[number].Name);
			return 1;
		}
		if (this->bot[number].ContributionGens > gObj[index].GensContribution)
		{
			//gNotice.NewMessageDevTeam(gObj[index].Index, gNewMessage.GetTextMessage(529, gObj[index].Language), this->bot[number].Name, this->bot[number].ContributionGens);
			return 1;
		}
	}
	...
	for (int x = 0; x < this->bot[number].SkillCount; x++)
	{
		...
		//gNotice.NewNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, gNewMessage.GetTextMessage(530,lpObj->Language), this->bot[number].Name, this->bot[number].skill[x].time);
	}
	...
	if (this->bot[number].Zen > 0)
	{
		...
		//gNotice.NewNoticeSend(gObj[index].Index, 0, 0, 0, 0, 0, gNewMessage.GetTextMessage(531, lpObj->Language), this->bot[number].Name, this->bot[number].Zen);
	}
	...
	if (this->bot[number].PCPoints > 0)
	{
		...
		//gNotice.NewNoticeSend(gObj[index].Index, 0, 0, 0, 0, 0, gNewMessage.GetTextMessage(532, lpObj->Language), this->bot[number].Name, gObj[index].PCPoint);
	}

	//gNotice.NewMessageDevTeam(lpObj->Index, gNewMessage.GetTextMessage(533, lpObj->Language), this->bot[number].Name);
	return 1;
}
