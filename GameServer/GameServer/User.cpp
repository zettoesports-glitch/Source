...
short gObjDel(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		if(lpObj->Account[0] != 0){ LogAddConnect(LOG_RED,"[ObjectManager][%d] DelAccountInfo (%s)",aIndex,lpObj->Account);}

		LogAddConnect(LOG_RED,"[ObjectManager][%d] DelClient (%s)",aIndex,lpObj->IpAddr);
