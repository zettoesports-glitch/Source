...
float waveHeight(vec2 p, float t, float detail)
{
    ...
    for (int i = 0; i < WAVE_COUNT; ++i)
    {
        ...
        height += sin(dot(p, WAVES[i].xy) * WAVES[i].z + t * WAVES[i].w) * amplitude;
    }
    return height;
}
...
vec2 rainRipples(vec2 p, float t, float intensity, out float crest)
{
    ...
    for (int x = -1; x <= 1; ++x)
    {
        ...
        gradient += (delta / max(distanceToDrop, 0.001)) * cos(wave * 0.55) * envelope;
...
vec2 footstepMark(vec2 p, out float wetMark, out float contactShine)
{
    ...
    for (int i = 0; i < FOOTSTEP_MAX; ++i)
    {
        ...
        float toe = footprintEllipse(localPosition, vec2(7.0, 0.0), vec2(16.0, 8.0));
        float middle = footprintEllipse(localPosition, vec2(-1.0, 0.0), vec2(12.5, 5.4));
        float heel = footprintEllipse(localPosition, vec2(-10.0, 0.0), vec2(10.0, 6.6));
        float sole = max(toe, max(middle * 0.88, heel * 0.96));
        wetMark += sole * fade;
        float toeBorder = footprintBorder(localPosition, vec2(7.0, 0.0), vec2(16.0, 8.0));
        float middleBorder = footprintBorder(localPosition, vec2(-1.0, 0.0), vec2(12.5, 5.4));
        float heelBorder = footprintBorder(localPosition, vec2(-10.0, 0.0), vec2(10.0, 6.6));
        float shineFade = pow(max(1.0 - age, 0.0), 0.85);
        float border = max(toeBorder, max(middleBorder * 0.80, heelBorder));
...
vec3 shadeWater(vec4 textureColor)
{
    ...
    vec2 gradient = waveGradient(p * (1.0 + thick * 0.55), weatherTime * (1.0 - thick * 0.45) * (1.0 - frost * 0.45), detail) * (22.0 * (1.0 - thick * 0.55));
    float crest = 0.0;
    if (rainSurfaceFxEnabled != 0 && rainIntensity > 0.001)
        gradient += rainRipples(p, weatherTime, rainIntensity, crest) * 4.5 * detail;
    ...
    float depth = clamp((waterLevel - worldPosition.z) / max(waterDepthRange, 1.0), 0.0, 1.0);
    depth = pow(depth, 0.7);
    float textureLuminance = dot(textureColor.rgb, vec3(0.299, 0.587, 0.114));
    vec3 clearBody = mix(WATER_SHALLOW, WATER_DEEP, depth) * (0.68 + 0.48 * textureLuminance);
    ...
    result = mix(result, WATER_SKY, fresnel * 0.48 * (1.0 - turbid * 0.88) * (1.0 + frost * 0.55));
    result += SUN_TINT * sparkle * (1.0 - turbid * 0.80) * (1.0 + frost * 1.10) + vec3(sheen) * (1.0 - turbid * 0.55) * (1.0 + frost * 0.85);
    ...
    float shore = smoothstep(0.015, 0.045, depth) * (1.0 - smoothstep(0.10, 0.22, depth));
    float churn = waveHeight(p * 2.4, weatherTime * 1.6, detail) * 0.16;
    float foam = smoothstep(0.58, 0.96, shore + churn * 0.55);
    vec3 foamColor = mix(vec3(0.91, 0.96, 0.98), result * vec3(0.72, 0.86, 0.62), turbid);
...
vec3 shadeWetGround(vec3 albedo)
{
    ...
    float distanceToEye = length(toEye);
    float detail = clamp(1.0 - (distanceToEye - 700.0) / 1500.0, 0.0, 1.0);
    vec3 wet = wetGroundFxEnabled != 0 ? albedo * mix(1.0, 0.68, rain * 0.9) : albedo;
    ...
    {
        ripple += footstepMark(worldPosition.xy, wetMark, contactShine) * 3.0 * detail;
    ...
    vec3 viewDirection = toEye / max(distanceToEye, 0.001);
    vec3 halfDirection = normalize(normalize(SUN_DIR) + viewDirection);
    float specular = pow(max(dot(normal, halfDirection), 0.0), 90.0) * 0.50 * rain * detail;
    wet *= mix(1.0, 0.48, wetMark);
    specular += pow(max(dot(normal, halfDirection), 0.0), 140.0) * 0.28 * wetMark;
    ...
    if (useTexture != 0)
    {
        ...
        if (weatherEnabled != 0 && terrainSurface == 2 && waterFxEnabled != 0)
        {
            if (waterLava > 0.5)
            {
                ...
                color = vec4(mix(color.rgb * lava, lava, 0.70), color.a * textureColor.a);
            }
            else
                color = vec4(color.rgb * shadeWater(textureColor), color.a * textureColor.a);
        }
        else if (weatherEnabled != 0 && terrainSurface == 1 && rainIntensity > 0.001
            && (wetGroundFxEnabled != 0 || rainSurfaceFxEnabled != 0 || footstepFxEnabled != 0))
            color = vec4(color.rgb * shadeWetGround(textureColor.rgb), color.a * textureColor.a);
        else
            color *= textureColor;
#else
        color *= textureColor;
