...
function ChangeClass.Command(aIndex, Arguments, Type)
    if COMMAND_CHANGE_CLASS_SWITCH == 0 then
        return
    end
    ...
    if player:getInterfaceUse() ~= 0 or player:getDieRegen() ~= 0 or player:getTeleport() ~= 0 then
        SendMessage(string.format(CHANGE_CLASS_MESSAGES[Language][1]), aIndex, 0)
        return
    end

    if player:getLevel() < COMMAND_CHANGE_CLASS_LEVEL then
        SendMessage(string.format(CHANGE_CLASS_MESSAGES[Language][2], COMMAND_CHANGE_CLASS_LEVEL), aIndex, 1)
        return
    end

    local money = player:getMoney()
    if money < COMMAND_CHANGE_CLASS_MONEY then
        SendMessage(string.format(CHANGE_CLASS_MESSAGES[Language][3], COMMAND_CHANGE_CLASS_MONEY), aIndex, 1)
        return
    end

    if player:getVip() < COMMAND_CHANGE_CLASS_VIP then
        SendMessage(string.format(CHANGE_CLASS_MESSAGES[Language][4]), aIndex, 0)
        return
    end

    if player:getReset() < COMMAND_CHANGE_CLASS_RESETS then
        SendMessage(string.format(CHANGE_CLASS_MESSAGES[Language][5], COMMAND_CHANGE_CLASS_RESETS), aIndex, 1)
        return
    end

    if player:getMasterReset() < COMMAND_CHANGE_CLASS_MRESETS then
    ...
    for i = 0, 11 do
        if pInv:isItem(i) == 1 then
            SendMessage(string.format(CHANGE_CLASS_MESSAGES[Language][7]), aIndex, 0)
            SendMessage(string.format(CHANGE_CLASS_MESSAGES[Language][8]), aIndex, 0)
            return
        end
    end
    ...
    if Type == 1 then -- SM
        if player:getClass() == CLASS_DW then
            SendMessage(string.format(CHANGE_CLASS_MESSAGES[Language][9]), aIndex, 0)
            return
        end
        class   = CLASS_DW
        dbclass = DB_CLASS_DW
    ...
    elseif Type == 2 then -- BK
        if player:getClass() == CLASS_DK then
    ...
    elseif Type == 3 then -- ELF
        if player:getClass() == CLASS_FE then
    ...
    elseif Type == 4 then -- MG
        if player:getClass() == CLASS_MG then
    ...
    elseif Type == 5 then -- DL
        if player:getClass() == CLASS_DL then
    ...
    elseif Type == 6 then -- SUM
        if player:getClass() == CLASS_SU then
    ...
    if refresh == 1 then
        -- Deleção da tabela CustomAttack
        local CharacterName = player:getName()
        local db = DataBase.getDb()
        if db then
            local query = string.format("DELETE FROM CustomAttack WHERE Name = '%s'", CharacterName)
            if db:exec(query) == 0 then
                LogAdd(string.format("Erro ao deletar CustomAttack para %s", CharacterName))
            end
            db:clear()
        end

        -- Aplicação da mudança de classe
        ...
        -- Desconexão do personagem (igual ao CharFull)
        --CloseChar(aIndex, 1)
...
Commands.Register(COMMAND_CHANGE_CLASS_SU,  ChangeClass.Command, 6)
