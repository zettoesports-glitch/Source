...
function RewardPlayer.Command(aIndex, Arguments)
	...
	then
		...
		Message.SendMessageGlobalMultLangArgs(REWARD_PLAYERS_GLOBAL_MESSAGES, 2, 0, TargetName, RewardAmount, rewardInfo.CoinName)
		SendMessageGlobal(string.format("========================"), 0)
		LogAdd(string.format("[GM Command] - [%s] adicionou %d %s para o Player %s", player:getName(), RewardAmount, rewardInfo.CoinName, TargetName), 0)
	end
